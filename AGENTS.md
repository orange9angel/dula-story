...
以下约束必须在写 `.story` 时主动遵守，不能依赖渲染后肉眼发现。
...
- **禁止行为**：不要让角色从桌椅内部“浮出来”或从家具阴影/后方高速滑入画面。

#### 2. 相机 vs 角色避碰
- **CloseUp 安全距离**：
  - 大雄 / 静香等标准体型角色：`distance ≥ 2.2`
  - 哆啦A梦 / 胖虎 / 大恐龙等宽大体型角色：`distance ≥ 2.5`
- **OverShoulder / TwoShot**：机位应放在所有被拍摄角色的后方或侧后方，距离至少覆盖两个角色间距 + 1.0 米冗余。
- **运动运镜（Pan / ZoomOut / Tracking）**：若起点和终点之间的连线会经过角色身体，改用 `Camera:Static` 或分段切换机位。
- **禁止行为**：相机不得从角色头、躯干内部穿出/穿入；CloseUp 不得把相机埋进角色身体里。

#### 3. 验证动作
每次修改后必须执行：
1. `npx dula-verify ./episodes/<name>` 生成 `storyboard/check_shot_*.jpg`。
2. 逐张检查是否有穿模、相机切入角色、角色从家具内部出现。
