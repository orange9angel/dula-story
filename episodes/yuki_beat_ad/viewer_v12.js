...
import { buildDanceRig, poseDance, applyLips, actFace, drawMotif } from './performance_v11.js';
...
await board.load('/episode/script_v12.story','/episode/assets/audio/manifest.json');
...
function draw(t) {
  ...
  return{index:entry.index,t,local,outfit,accessory,expression,gesture,move,roll,airborne:Math.round(posed.airborne*100)/100,
    ...
    lip:lipAt(t),mouthVisible:c.mouthCavity.visible,mouthScale:c.mouthCavity.scale.toArray(),
...
if(!new URLSearchParams(location.search).has('capture')){
  const audio=new Audio('/episode/assets/audio/mixed_v12.wav');audio.loop=true;
