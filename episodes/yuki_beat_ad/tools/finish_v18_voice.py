"""V18 step 3: rebuild the master with the converted Yuki voice.

mixed_v18 = base_master - original_vocals + converted_vocals * gain
...
def main():
    ...
    base = load(ROOT / 'assets/audio/mixed_v18_base.wav')
    vocals = load(ROOT / 'assets/audio/stems_v18/vocals.wav')
    ...
    target = ROOT / 'assets/audio/mixed_v18.wav'
    with wave.open(str(target), 'wb') as w:
        w.setnchannels(2); w.setsampwidth(2); w.setframerate(SR)
        w.writeframes((out * 32767).astype('<i2').tobytes())
    shutil.copyfile(target, ROOT / 'assets/audio/music/mixed_v18.wav')
    sf.write(ROOT / 'assets/audio/voice_v18/vocals_vc_dry.wav', dry_g * common, SR, subtype='PCM_16')

    report = {
        'song': 'assets/audio/music/v18/candidate_d.wav',
        'base_master': 'assets/audio/mixed_v18_base.wav',
        'vocals_stem': 'assets/audio/stems_v18/vocals.wav',
