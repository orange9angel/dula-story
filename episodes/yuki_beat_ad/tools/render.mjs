...
const version = v15 ? 'v15' : v14 ? 'v14' : v13 ? 'v13' : v12 ? 'v12' : v11 ? 'v11' : v10 ? 'v10' : v9 ? 'v9' : v8 ? 'v8' : v6 ? 'v6' : v5 ? 'v5' : v4 ? 'v4' : v3 ? 'v3' : v2 ? 'v2' : '';
const boardDir = path.join(root, version ? `storyboard/${version}` : 'storyboard');
const audioFile = version==='v15' ? 'assets/audio/mixed_v13.wav' : version==='v14' ? 'assets/audio/mixed_v13.wav' : version ? `assets/audio/mixed_${version}.wav` : 'assets/audio/mixed.wav';
const outputFile = version ? `output/yuki_beat_ad_${version}.mp4` : 'output/yuki_beat_ad.mp4';
const fps = v3 || v4 || v5 || v6 || v8 || v9 || v10 || v11 || v12 || v13 || v14 || v15 ? 60 : 30;
...
if (!serveOnly) {
  ...
  try {
    ...
    for(let i=0;i<Math.round(duration*fps);i++) {
      ...
      if(v10 || v11 || v12 || v13 || v14 || v15) performanceTrace.push({frame:i,...(capture?result.state:result)});
      if(capture) {
    ...
    if(v10 || v11 || v12 || v13 || v14 || v15) fs.writeFileSync(path.join(boardDir,'performance_trace.json'),JSON.stringify({duration,fps,frames:performanceTrace}));
