"""Align known lyrics via Whisper cross-attention/DTW on the separated vocal.

This is character alignment, not measured phoneme boundaries. Encode characters
separately so merged BPE words do not get equal-duration fabricated splits.
"""
...
def main():
    ...
    lines = [s.strip() for s in (ROOT / 'config/diva_lyrics.txt').read_text(encoding='utf-8').splitlines() if s.strip()]
    text = ''.join(lines).replace(' ', '')
    target = ROOT / 'config/lyrics_forced_alignment_v10.json'
