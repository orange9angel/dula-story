...
const music=JSON.parse(fs.readFileSync(path.join(root,'config/music_analysis_v19.json'),'utf-8'));
const story=fs.readFileSync(path.join(root,'script_v19.story'),'utf-8');
...
const server=spawn(process.execPath,[path.join(root,'tools/render.mjs'),'--v19','--serve'],{stdio:['ignore','pipe','pipe']});
let browser;
try{
  ...
  await page.goto('http://127.0.0.1:4188/viewer_v19.html?capture=1',{waitUntil:'networkidle0'});
  await page.waitForFunction('window.ready === true',{timeout:60000});
  const folder=path.join(root,'tmp/mouth_probe_v19');fs.mkdirSync(folder,{recursive:true});
  const report=[];
  ...
  const frames=[['bu_seal',bu.start+.01],['bu_purse',bu.start+.075],
