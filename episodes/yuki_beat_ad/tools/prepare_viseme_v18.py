"""V18: V17 viseme track method re-generated for the new song.
...
def main():
    ...
    features = json.loads((ROOT / 'config/vocal_features_v18.json').read_text(encoding='utf-8'))
    ...
    report = {
        'sample_step': STEP,
        'keyframes': track,
        'method': ('V17 track method on the V18 song: purse=1 on u/ü vowels and ou/ao/iu '
        ...
        'source': 'music_analysis_v18.json lyric_chars + vocal_features_v18 amplitude',
    }
    (ROOT / 'config/viseme_track_v18.json').write_text(
        json.dumps(report, ensure_ascii=False, indent=2), encoding='utf-8')
    print(f'V18 viseme track: {len(track)} keyframes over {duration}s from {len(chars)} characters')
    for c in chars:
