"""V19: append Mochi's backup-dancer lane to script_v19.story.

prepare_v16 logic against the V19 data: Yuki entries are kept verbatim,
Mochi entries are re-timed to the V19 beat grid / section roles and appended
...
def main():
    ...
    story_text = (ROOT / 'script_v19.story').read_text(encoding='utf-8')
    ...
    out = '\n\n'.join(yuki_blocks) + '\n\n' + '\n\n'.join(blocks) + '\n'
    (ROOT / 'script_v19.story').write_text(out, encoding='utf-8')
    print(f'V19: {len(yuki)} Yuki entries kept verbatim, {len(blocks)} Mochi entries appended '
