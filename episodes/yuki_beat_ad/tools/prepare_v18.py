"""V18: full re-analysis of the new song + lyric choreography (Yuki lane).

Chain (v9/v10/v11/v12/v14 methods, V18 paths; earlier-version files untouched):
  ...
  -> Whisper DTW alignment       (align_v18, 80-char lyric text asserted)
  -> vocal_data                  (prepare_v10 method on stems_v18)
  -> spectral-flux calibration   (calibrate_v18; audio_start/audio_end per char)
  -> anti-vibrato vocal features (prepare_v14 method -> vocal_features_v18.json)
  -> V11 semantic performance cues re-timed to the new alignment
Outputs: script_v18.story (Yuki lane only; Mochi lane is appended by
prepare_v18_mochi.py), config/music_analysis_v18.json,
config/performance_plan_v18.json, config/vocal_features_v18.json.
"""
...
from align_v18 import main as align
from calibrate_v18 import calibrate

ROOT = Path(__file__).resolve().parents[1]
MASTER = ROOT / 'assets/audio/mixed_v18.wav'
STEM = ROOT / 'assets/audio/stems_v18/vocals.wav'
ANA_SR = 22050


def vocal_data():
    """prepare_v10 vocal_data on the V18 stem/alignment."""
    aligned = json.loads((ROOT / 'config/lyrics_forced_alignment_v18.json').read_text(encoding='utf-8'))
    ...
    for li, line in enumerate(original):
        ...
        for c in group:
            c['line'] = li
        index += count
    assert index == len(chars) == 80, f'歌词覆盖断言失败: {index} chars, 期望 80'
    ...
    return chars, lines, envelope, {'floor': float(floor), 'reference_rms': float(ref),
        ...
        'raw_alignment': 'config/lyrics_forced_alignment_v18.json',
...
def drum_grid(grid, old_beats_key='beats'):
    ...
    y, sr = sf.read(ROOT / 'assets/audio/stems_v18/drums.wav', always_2d=True)
    ...
    return {**grid, 'beats': beats, 'downbeats': db,
            'method': 'V18 librosa grid refined to isolated-drum attack within 120ms',
            'kick_attacks': [round(a, 3) for a in attacks]}


def vocal_features():
    """prepare_v14 anti-vibrato amplitude envelope on the V18 vocal stem."""
    HOP = .01
    y, sr = sf.read(STEM, always_2d=True)
    ...
    report = {'method': '20ms centered RMS / 10ms hop -> asymmetric envelope (20ms attack / 250ms release) -> ~100ms low-pass, 99th-percentile reference, dynamic amplitude; high-band ratio for consonant support',
        'source': 'assets/audio/stems_v18/vocals.wav',
    ...
    (ROOT / 'config/vocal_features_v18.json').write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding='utf-8')
    print(f'V18 vocal features: {n} frames; saturation {sum(amplitude >= .999)/n:.1%}')


def choreography(music, grid, duration):
    """V11 semantic performance cues, re-timed to the V18 alignment."""
    ...
    for i, (start, move, li, emotion, focus, motif, hit, shot, intent) in enumerate(specs):
        ...
        prefix = ('@BeatStudioScene\n{Position:Yuki|x=0|y=-0.03|z=0|face=forward}\n'
                  f'{{Music:Play|name=mixed_v18|endTime={duration}|baseVolume=1|fadeIn=0|fadeOut=0}}\n') if i == 0 else ''
...
def main():
    assert MASTER.exists(), '先运行 finish_v18_voice.py 生成 mixed_v18.wav'
    duration = round(sf.info(MASTER).duration, 3)
    base_duration = round(sf.info(ROOT / 'assets/audio/mixed_v18_base.wav').duration, 3)
    ...
    align()  # writes config/lyrics_forced_alignment_v18.json (80-char text asserted)
    ...
    (ROOT / 'config/lipsync_calibration_v18.json').write_text(
    ...
    (ROOT / 'script_v18.story').write_text(story, encoding='utf-8')

    y_master = load(MASTER)
    report = dict(source='assets/audio/music/v18/candidate_d.wav',
        ...
        choreography=cues, timeline_source='script_v18.story',
        performance_plan='config/performance_plan_v18.json',
        review_limit='Model-assisted checks, no human playback acceptance or exact phoneme-sync claim')
    (ROOT / 'config/music_analysis_v18.json').write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding='utf-8')
    plan = dict(audio_analysis='config/music_analysis_v18.json', master_sha256=report['master_sha256'],
                duration=duration, timeline_source='script_v18.story', cues=cues,
                sections=report['sections'],
                review_limit='Authored expressiveness and semantic gestures require user playback review.')
    (ROOT / 'config/performance_plan_v18.json').write_text(json.dumps(plan, ensure_ascii=False, indent=2), encoding='utf-8')
    vocal_features()
    print(f'V18: {duration}s, BPM {grid["bpm"]}, {len(chars)} aligned characters, {len(cues)} performance phrases')
    for c in cues:
