"""Calibrate lyric timing against a vocal stem, preserving uncertainty/provenance.

A periodic rise score is a hypothesis about lag, not phoneme ground truth.
Accept a global shift only with a distinct peak and independent phrase agreement.
Locally match one vocal attack to one character, with order and duration guards.
"""
...
def calibrate(music):
    ...
    master=ROOT/'assets/audio/mixed_v11.wav'
    sep=json.loads((stem.parent/'separation.json').read_text(encoding='utf-8'))
    assert digest(master)==music['master_sha256']==sep['source_sha256']
    ...
    report={'vocal_stem_sha256':digest(stem),'master_sha256':digest(master),
        'source_analysis_sha256':digest(ROOT/'config/music_analysis_v10.json'),
        ...
        'scan':[{'offset_ms':round(off*1000),'flux_score':round(float(f),7),'energy_rise_score':round(float(r),7)} for off,f,r in zip(offsets,fs,rs)],
...
if __name__=='__main__':
    m=json.loads((ROOT/'config/music_analysis_v10.json').read_text(encoding='utf-8'))
    chars,lines,report=calibrate(m)
    print(json.dumps({k:v for k,v in report.items() if k not in ('scan','characters','attacks','inherited_overlaps','limits')},ensure_ascii=False,indent=2))
