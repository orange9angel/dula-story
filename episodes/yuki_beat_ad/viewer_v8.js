...
await board.load('/episode/script_v8.story','/episode/assets/audio/manifest.json');
const music=await(await fetch('/episode/config/music_analysis_v8.json')).json();
...
{ // Check frames: every change (before/attack/hold) plus one frame per V6/V8 element
  ...
  const ghosts=board.entries.filter(e=>on(optsOf(e).ghost)).flatMap(e=>[e.startTime+.6,(e.startTime+e.endTime)/2]);
  ...
  window.checkTimes=[0,...board.entries.slice(1).flatMap(e=>[(renderFrame(e.startTime)-1)/60,renderFrame(e.startTime)/60,renderFrame(e.startTime)/60+.2]),
    ...snaps,...swipes,...cards,...ghosts,...vocals,window.duration-.1];
...
function updateStage(stage,t,pal,pulse){
  ...
  stage.floaters.forEach(m=>{
    ...
    if(stage.theme==='candy'){
      ...
      m.position.y=u.base.y+Math.sin(t*u.speed+u.phase)*.06;
      u.glow.material.opacity=.10+.10*(.5+.5*Math.sin(t*u.speed*2+u.phase));
    } else {
      m.position.x=u.base.x+Math.sin(t*u.speed+u.phase)*.1;
      m.position.y=u.base.y+Math.sin(t*u.speed*.7+u.phase)*.04;}
  });
  ...
  if(stage.extras.spotlights)stage.extras.spotlights.forEach((s,i)=>{
    ...
    s.material.opacity=.08+.05*pulse+.03*(.5+.5*Math.sin(t*2.4+i*2));});
  if(stage.extras.dust)stage.extras.dust.material.opacity=.5+.3*(.5+.5*Math.sin(t*1.7));
  stage.beams.forEach((b,i)=>{
    b.rotation.z=(i?1:-1)*(.42+.2*Math.sin(t*.7+i*2));
    b.material.opacity=(stage.theme==='candy'?.09:.06)+.06*pulse;});
...
function poseFull(c,opts,entry,t,pulse=0){
  ...
  const breath=last&&local>.2?0:Math.sin(local*5)*.006;
  ...
  c.leftTail.rotation.z=c.leftTail.userData.baseRotZ-.065*Math.sin(t*8);
  c.rightTail.rotation.z=c.rightTail.userData.baseRotZ+.065*Math.sin(t*8);
  c.ahoge.rotation.z=.6+pulse*.22;
  ...
  if(move==='twirl'){
    ...
    // with the beat pulse, body bounces on each onset like 'dance'.
    const beats=music.onsets.filter(o=>o.time>=entry.startTime-.01&&o.time<t+.01);
    const lastBeat=beats.length?beats[beats.length-1].time:entry.startTime;
    const bounce=Math.exp(-(t-lastBeat)*9);
    const sway=Math.sin(local*3.2);
    c.mesh.position.y+=.04*bounce;
    c.mesh.rotation.y=.10*sway;
    c.headGroup.rotation.z+=.06+.05*sway;
    ...
    // Arms trade wide sweeps across the chest, alternating on every onset.
    const beats=music.onsets.filter(o=>o.time>=entry.startTime-.01&&o.time<t+.01);
    const parity=beats.length%2;
    const lastBeat=beats.length?beats[beats.length-1].time:entry.startTime;
    ...
    c.headGroup.rotation.z+=lead*.08;
    if(lead>0){
  ...
  // V8 singing mouth: inside vocal_env windows the sung mouth wins over the
  // expression mouth; 'closed' falls back to whatever the expression set up.
  const vocal=mouthAt(t);
  if(vocal!=='closed'){
    const open01=vocal==='open'?1:.5;
    c.upperLip.visible=false;
    c.lowerLip.visible=false;
    c.beatFace.smile.visible=false;
    c.beatFace.tongue.visible=vocal==='open';
...
const cards=board.entries.filter(e=>optsOf(e).card!=='none')
  .map(e=>({t0:e.startTime,text:cardTexts[optsOf(e).card]}));
...
function draw(t) {
  ...
  const posed=poseFull(c,opts,entry,t,state.pulse);
  ...
  label('小雪 YUKI',360,64,23,shownScene==='candy'?'#3d3044':'#ffffff');
  ...
  return{index:entry.index,t,local,outfit,accessory,expression,gesture,move,roll,airborne:Math.round(posed.airborne*100)/100,
    ...
    vocal:mouthAt(t),lyric,mic:mic.visible,
...
if(!new URLSearchParams(location.search).has('capture')){
  const audio=new Audio('/episode/assets/audio/mixed_v8.wav');audio.loop=true;
