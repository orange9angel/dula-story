...
await board.load('/episode/script_v13.story','/episode/assets/audio/manifest.json');
const music=await(await fetch('/episode/config/music_analysis_v12.json')).json();
const visemeTrack=await(await fetch('/episode/config/viseme_track_v15.json')).json();
const plan=await(await fetch('/episode/config/performance_plan_v11.json')).json();
...
// Character-aligned lyrics; the mouth is driven by the baked V15 viseme track.
// Keyframes carry continuous [jaw, width(-1..1), rounding, seal, labiodental,
// amplitude]; every channel is cosine-interpolated between the 10ms-aligned
// keyframes, so articulation moves continuously through consonant->vowel->
// silence instead of stepping per character. width is remapped to the
// applyLips convention (1.0 = neutral) for the shared V13 mouth geometry.
const lipAt=(()=>{
  ...
  return t=>{
    ...
    return {shape:ch?ch.final.toUpperCase():'rest',open:jaw,jaw,
      ...
      seal:chan(t,'seal'),labiodental:chan(t,'labiodental'),amplitude:chan(t,'amplitude'),
...
function poseFull(c,opts,entry,t){
  const pe=board.entries[board.entries.indexOf(entry)-1];
  ...
  posed.mouth=applyLips(c,lipAt(t));  const faceAmp=sectionAt(t).role==='chorus'?1.3:1;
  ...
  const lip=lipAt(t);
  posed.mouth=applyLips(c,{...lip,width:lip.width*(1+.35*(faceAmp-1))*(opts.move==='finale'?1.12:1)});
  return posed;
}
...
window.renderLipProbe=(t,lip)=>{
  draw(t);const c=board.characters.get('Yuki');const mouth=applyLips(c,lip);
...
if(!new URLSearchParams(location.search).has('capture')){
  const audio=new Audio('/episode/assets/audio/mixed_v13.wav');audio.loop=true;
