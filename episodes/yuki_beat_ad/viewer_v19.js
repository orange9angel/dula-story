...
await board.load('/episode/script_v19.story','/episode/assets/audio/manifest.json');
const music=await(await fetch('/episode/config/music_analysis_v19.json')).json();
const visemeTrack=await(await fetch('/episode/config/viseme_track_v19.json')).json();
const plan=await(await fetch('/episode/config/performance_plan_v19.json')).json();
...
if(!new URLSearchParams(location.search).has('capture')){
  const audio=new Audio('/episode/assets/audio/mixed_v19.wav');audio.loop=true;
