...
## V19 新词新曲（2026-09-19，待播放评审）

成片：`output/yuki_beat_ad_v19.mp4`（29.4 秒，720×1280，60 fps）。
应监制要求歌词歌曲全换：新词按起承转合写（踮脚听拍→张臂点星→跟上这一拍→
左右步→跳转→小雪登场→世界跟我发光→下一拍你登场），保留编舞锚点字；
新曲四候选客观评分选出 candidate_d（Pop/Happy/Vivace，钢琴+合成器+鼓组，
143.55 BPM，6.77 分）。小雪本音 Seed-VC 转换（包络互相关零延迟）。
编舞按新词逐句重排（踮脚/侧耳/点星/空气鼓/星光桥/收尾欢呼）。
全链路断言通过。本轮模型成本：4 候选 + 2 次参数试探 ≈ 0.4 元。

附：song_gen.py 新增 --tempo/--key/--instrument（v4.3 结构参数；
Tempo 用意大利速度术语如 Vivace，Instrument 用英文枚举如
Acoustic_Piano,Synthesizers,Drums）。

## V18 起承转合音乐版 + 小雪本音（2026-09-19，待播放评审）

成片：`output/yuki_beat_ad_v18.mp4`（29.5 秒，720×1280，60 fps）。
针对"音乐平顺、要起承转合"的重制：**歌词不变**（保住编舞锚点），音乐重选。

- **选曲**：4 候选（Pop/电子混合 × 不同情绪音色）经 `tools/score_v18.py`
  客观评分——段落能量对比/蓄力斜率/爆发落差/动态范围/拍点清晰度/配器变化
  六项加权，candidate_d（Electronic+Pop, Dynamic/Energetic, Bright）
  以 8.04 分胜出（V8 原曲基线 3.94）。新曲 136 BPM、五段式能量弧。
- **小雪本音**：Demucs 分离新曲人声 → Seed-VC 零样本转换（steps=30,
  cfg=0.65，参考《早起大作战》小雪配音声线）→ 增益匹配重组母带
  （包络互相关验证零延迟，逐采样等长）。
- **链路重跑**：新拍网（含尾段鼓退出后的拍网外推标记）→ DTW 逐字对齐 →
  起音锚定/频谱校准 → 连续视素轨道（315 关键帧）→ Mochi lane 重排，
  全部断言通过（80 字覆盖、静音闭嘴、人猫零穿模、0 浏览器错误）。
- 本轮模型成本：4 首候选 ≈ 0.24 元。
- 另修：`song_gen.py` 中转改 URL 下载优先（base64 大 JSON 偶发截断），
  并发现歌曲 CDN 下载不受地域限制。
...
针对"V14 口型变化不够连续"：口型从逐字离散视素升级为**关键帧轨道 +
连续插值**（`prepare_v15.py` → `config/viseme_track_v15.json`，327 个
关键帧）。每字拆辅音段/元音段，元音用连续 jaw/width/rounding 目标，
复元音滑动、辅音段 40% 协同预摆下一元音、静默 100ms 缓闭；
viewer_v15 余弦插值连续求值。jaw 加加速度（jerk）较 V14 降 81%；
探针实测双唇音先闭合后释放、a→i 滑音中间态可见。修复了 DTW 字窗重叠
（32/80 字）导致的轨道交错。其余（字幕/舞台/表演/音频）与 V14 一致。
