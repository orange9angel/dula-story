...
await board.load('/episode/script_v6.story','/episode/assets/audio/manifest.json');
const music=await(await fetch('/episode/config/music_analysis_v6.json')).json();
...
const on=v=>v===1||v==='1';
{ // Check frames: every change (before/attack/hold) plus one frame per V6 element
  ...
  window.checkTimes=[0,...board.entries.slice(1).flatMap(e=>[(renderFrame(e.startTime)-1)/60,renderFrame(e.startTime)/60,renderFrame(e.startTime)/60+.2]),
    ...snaps,...swipes,...cards,...ghosts,window.duration-.1];
...
// --- V6: three scene themes, same flat-color + glowing-geometry language ---
...
function poseFull(c,opts,entry,t,pulse=0){
  ...
  // Dance moves, layered over the base pose (same as V5)
  let airborne=0;
  if(move==='twirl'){
...
if(!new URLSearchParams(location.search).has('capture')){
  const audio=new Audio('/episode/assets/audio/mixed_v6.wav');audio.loop=true;
