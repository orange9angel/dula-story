#!/usr/bin/env python3
"""Generate CosyVoice dialogue, manifest, dialogue bus, and the full 60 s mix.
...
4. Schedule SFX from script.story {SFX:Play} tags, build the BGM bus from
   assets/audio/music/morning_guitar_lesfm_11157.mp3, and mix
   assets/audio/mixed.wav (60.0 s, 44.1 kHz) using config/audio_mix.json.
...
MUSIC_PATH = next((OUTPUT_DIR / "music").glob("*.mp3"), None)  # dusk BGM, dropped in by the BGM selection step
...
# explicit endTime in script.story (room_tone_day, summer_cicadas,
# store_ambience) are trimmed by the endTime path instead.
SFX_TRIM = {
    "cat_meow": (1.6, 0.35),
    "cat_purr": (3.0, 0.6),
...
def build_bgm_bus() -> None:
    # Envelope: quiet bed under the bank/bridge acts, a swell that
    # reaches full gain at the closing line (storyboard beat at 50.3 s), a
