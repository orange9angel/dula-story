#!/bin/bash
# E03 mouth/eye variants: ALL via codex local edit (the only passing craft —
# E02 V2-V4 verdict: qwen/wanx/procedural all failed on paste-back cels).
# Girl mouths x5 (half+open), cat meow mouths x3 (half+open),
# girl blinks x7, cat blinks x3. Run after all keyframes exist.
set -u
...
# Girl monologue mouths (entries 2/5/7/11/16)
...
# Cat meow mouths (entries 3/8/12)
for f in frame_02 frame_08 frame_12; do
  mouth "橘猫" "$f" half "微张（猫叫起音，吻部小幅打开，尺寸克制）"
  mouth "橘猫" "$f" open "自然张开（猫叫的开口，露出一点口腔，尺寸克制不要夸张）"
done

# Girl blinks
for f in frame_01 frame_03 frame_04 frame_06 frame_11 frame_14 frame_16; do
  eyes "$f" "少女"
done
