...
// E07《柳树下的老钓友》 daytime post layers. Per STYLE_BIBLE.md every procedural
...
// Colors come straight from the sunprint day palette: clear cyan-blue sky top,
// pale aqua horizon, flat gold dapple/light, lavender shadow purple, flat
// warm-white clouds, mid-green foliage.
const DAY = {
  skyTop: '#2E9BD6', // clear daytime cyan blue at the top of the sky
  horizon: '#7FD4E8', // pale aqua softening toward the horizon
  gold: '#F5B942', // flat gold: dapple spots, light bands, warm accents
  shadowPurple: '#8E7CC3', // lavender purple shadow side, never grey
  cloud: '#FDFBF4', // flat warm-white clouds / sparkles / wisps
  leaf: '#58A05C', // mid-green foliage for tree-canopy framing shapes
};
...
function parseStory(storyText) {
  ...
  if (!(duration > 0)) {
    throw new Error('WillowFishingSequenceScene could not read a positive duration from script.story');
  }
...
// "alpha": 0.32 } — flat gold daylight ellipses with a gentle
 ...
 * the E07《柳树下的老钓友》 daytime variant of the E03 keyframe-cel renderer.
 *
 ...
 *
 * All ambient motion (cloud drift, daylight dapple sway, steam, the door
 ...
 * stay deterministic.
 */
export class WillowFishingScene extends SceneBase {
  constructor() {
    super('WillowFishingScene');
    this.timeline = [];
  ...
  build() {
    ...
    this.sequenceTexture.name = 'WillowFishingSequenceTexture';
  ...
  _renderShotSource(frame, sequenceTime) {
    ...
    // Daytime ambient layers, drawn in source-image space so the crop move
  ...
  // Daytime bubble cloud: same flat-based body with four uneven bumps as E03,
  ...
  _drawCloudDrift(config, sequenceTime, sourceWidth) {
    ...
    for (let index = 0; index < config.clouds.length; index += 1) {
      ...
      ctx.fillStyle = DAY.shadowPurple;
      ctx.globalAlpha = 0.32;
      cluster(ry * 0.34, 0.6);

      ctx.fillStyle = DAY.cloud;
  ...
  // Daylight dapple: flat pale-gold ellipses (same language as the E03
  ...
  // front door opens and collapses on the entry shot. In daylight the
