...
import {twoBone,samplePose3D} from '/craft/pose3d.js';
...
export function bookLocal(p){return {position:V(0,p.hipY+.245,.35),quaternion:new THREE.Quaternion().setFromEuler(new THREE.Euler(-Math.PI/2+.16,0,0))};}
...
export function sampleKid(kind,t,plan,wind){
  ...
  let rootX=girl?-.68:.52,rootZ=girl?.46:.18,yaw=girl?.88:-.58,hipY=girl?.6925:.4025;
  const p={t,rootX,rootZ,yaw,hipY,bob:hipY-.6925,point:0,anticipation:0,headYaw:0,headPitch:0,headRoll:0,
    mouth:mouthAt(plan,kind,t),emotion:'calm',windWorld:wind,leftContact:true,rightContact:true,armLengths:girl?[.19,.18]:[.22,.21]};
  let feet=[V(-.086,.095,girl?0:.40),V(.086,.095,girl?0:.40)];
  if(girl&&t<B.arrived){
    const u=clamp(t-B.arrival,0,B.arrived-B.arrival-.35),walk=samplePose3D(u,'walk',B.arrived-B.arrival-.35);
    Object.assign(p,walk,{t,rootX:walk.rootX-1.11,rootZ,yaw:Math.PI/2,windWorld:wind,mouth:mouthAt(plan,kind,t),emotion:'curious'});
    if(t<B.arrival){const still=samplePose3D(0,'walk',1.65);Object.assign(p,still,{t,rootX:-1.40,rootZ,yaw:Math.PI/2,windWorld:wind,mouth:0,emotion:'curious'});}
    const turn=smooth((t-(B.arrived-.35))/.35);p.yaw=mix(Math.PI/2,.88,turn);
    return p;
  }
  ...
  p.bodyTilt=girl?0:.14;
  p.bodyRoll=girl?0:.12;
  ...
  for(const [i,side,sign] of [[0,'left',-1],[1,'right',1]]){
    const hip=V(sign*.075,p.hipY),shoulder=V(sign*.175,1.075-.70).applyEuler(new THREE.Euler(p.bodyTilt,0,p.bodyRoll)).add(V(0,.70+p.bob,0));
    p[side+'Leg']=solve(hip,feet[i],V(0,0,1),.305,.295);
