...
const fps = 30;
const boardDir = path.join(root, 'painted/storyboard');
const outputFile = path.join(root, 'painted/output/output.mp4');
...
const server = http.createServer((req,res) => {
  ...
  const mount = url.startsWith('/node_modules/') ? storyRoot : root;
  const target = path.resolve(mount, url.replace(/^\//,'') || 'painted/viewer.html');
  if (!target.startsWith(mount)) {res.writeHead(403).end();return;}
...
server.listen(serveOnly ? 4189 : 0,'127.0.0.1');
await once(server,'listening');
const url = `http://127.0.0.1:${server.address().port}/painted/viewer.html`;
console.log(url);
if (!serveOnly) {
  let browser, encoder;
  try {
    ...
    if (!check) {
      encoder=spawn('ffmpeg',['-y','-v','error','-f','image2pipe','-framerate',String(fps),'-vcodec','mjpeg','-i','pipe:0','-i',path.join(root,'assets/audio/mixed.wav'),'-map','0:v:0','-map','1:a:0','-c:v','libx264','-preset','medium','-crf','18','-pix_fmt','yuv420p','-c:a','aac','-ar','48000','-ac','2','-b:a','192k','-t',String(duration),'-movflags','+faststart',outputFile],{stdio:['pipe','inherit','inherit']});
    }
    for(let i=0;i<Math.round(duration*fps);i++) {
      ...
      if(i%150===0) console.log(`${check?'check':'render'} ${i}/${Math.round(duration*fps)}`);
    }
    if(encoder){encoder.stdin.end();const [code]=await once(encoder,'close');if(code!==0)throw new Error(`ffmpeg failed: ${code}`);}
    if(errors.length)throw new Error(errors.join('\n'));
    fs.writeFileSync(path.join(boardDir,'trace.json'),JSON.stringify({duration,fps,errors,shots:trace},null,2));
    console.log(check?'Shot checks complete':'Video complete');
