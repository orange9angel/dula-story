...
转发请使用 [op_wechat.mp4](op_wechat.mp4)：MP4，H.264 Main 4.0，
1920×1080、恒定 30 fps、方形像素、YUV 4:2:0；音频 AAC-LC、48 kHz、
双声道、192 kbps。音画时间戳从零开始，MP4 索引放文件头。

监制反馈经 QQ 转微信后变慢、音调未明显改变。原片实测为 96 kHz AAC，
音画起点均为 0，时长约 71.07s，包时间戳连续。96 kHz 是兼容性嫌疑，
尚不能认定它就是原因；音调不变不符合简单的 96→48 kHz 误读。
本版同时标准化音画参数，正常重采样，没有加速、减速或改音高。

本地验证见 [share_export_review.json](share_export_review.json)：媒体参数、
2132 帧、完整解码与四个不同时间窗的音频对齐；QQ / 微信实际往返尚待复测。
完整剪辑脚本也固定输出 48 kHz AAC-LC，防止后续导出继承非常用采样率。

```powershell
.venv\Scripts\python.exe episodes\cat_leads_e08_drifting_page\tools\export_op_share.py
```
