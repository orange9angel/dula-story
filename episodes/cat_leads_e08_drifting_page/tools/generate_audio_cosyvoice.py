#!/usr/bin/env python3
"""Generate dialogue (dual-provider), manifest, dialogue bus, and the 60 s mix.

Two TTS providers, selected per character via config/voice_config.json:

- Characters whose config carries ``"provider": "seedtts"`` (E04: Cat) go
  through Volcano seed-tts-2.0 via tools/seedtts_say.py (emotion /
  emotion_scale are passed through; ``rate`` is NOT supported by the
  unidirectional V3 HTTP API and is ignored with a warning). Requires
  VOLC_SPEECH_API_KEY.
- All other characters (E04: Girl) keep the CosyVoice DashScope
  HttpSpeechSynthesizer path (cosyvoice-v3-flash); non-stream calls return a
  signed temporary ``audio_url`` which is downloaded immediately and never
  persisted anywhere. Requires DASHSCOPE_API_KEY.

Both paths end as a clean 48 kHz mono PCM16 WAV per line, so measurement,
manifest and the buses downstream are provider-agnostic.

Pipeline (mirrors the E03 template episode mixer):

1. Parse script.story; synthesize every spoken entry ([Girl]/[Cat]).
...
4. Schedule SFX from script.story {SFX:Play} tags, build the BGM bus from the
   first *.mp3/*.wav under assets/audio/music/ (night piece, dropped in by
   the BGM selection step), and mix assets/audio/mixed.wav (60.0 s, 44.1 kHz)
   using config/audio_mix.json.
...
MUSIC_PATH = next(
    iter(sorted([*MUSIC_DIR.glob("*.mp3"), *MUSIC_DIR.glob("*.wav")])), None
)  # night BGM, dropped in by the BGM selection step
...
# recordings are much longer than the beats they score. Long beds with an
# explicit endTime in script.story (night_crickets, night_wind, river_water)
# are trimmed by the endTime path instead.
SFX_TRIM = {
    "cat_meow": (1.6, 0.35),
    "cat_purr": (3.0, 0.6),
    "footsteps_grass": (3.2, 0.6),
    "footsteps_stone": (3.1, 0.5),
    "footsteps_lane": (2.1, 0.4),
    "door_open": (1.1, 0.25),
    # E05 additions
    "sketchbook_pages": (2.6, 0.4),
    "birds_flyby": (3.5, 0.8),
    # E06 additions
    "pencil_scratch": (2.2, 0.3),
    # E07 additions
    "fishing_cast": (2.2, 0.4),
}
...
def resolve_voice_params(cfg: dict, emotion: str | None) -> dict:
    ...
    does not set falls back to the character's ``default`` block. Works for
    both providers (CosyVoice model/voice/rate/... and seedtts
    speaker/emotion/emotion_scale/...)."""
...
def synthesize_line_cosyvoice(text: str, params: dict, output_path: Path, api_key: str) -> None:
...
def synthesize_line_seedtts(text: str, params: dict, output_path: Path, api_key: str) -> None:
    """Cat's path: Volcano seed-tts-2.0 via tools/seedtts_say.py.

    seed-tts returns mp3 (per voice_config format/sampleRate); it is then
    re-encoded to the same 48 kHz mono PCM16 WAV convention as the CosyVoice
    path so everything downstream (measurement, manifest, buses) is unchanged.
    ``rate``/``pitch``/``volume`` have no counterpart in the unidirectional
    V3 HTTP API: rate is ignored with a warning, pitch/volume silently.
    """
    ...
    temp_path = output_path.with_suffix(".seedtts.mp3")
...
def synthesize_line(text: str, params: dict, output_path: Path, api_keys: dict) -> None:
    if params.get("provider") == "seedtts":
        synthesize_line_seedtts(text, params, output_path, api_keys["seedtts"])
    else:
        synthesize_line_cosyvoice(text, params, output_path, api_keys["cosyvoice"])
...
def build_bgm_bus() -> None:
    if MUSIC_PATH is None:
        raise RuntimeError(
            f"No BGM file (*.mp3/*.wav) found in {MUSIC_DIR}; "
            "drop the selected night piece there first."
        )
    # Envelope: quiet bed under the walk-out acts, a swell that reaches full
    # gain at the closing line ([Girl] "以后，也带我去吧。" starts 52.3 s), a
    # short hold, then a fade-out that lands at zero by 60 s. Gains here are
    # relative; config/audio_mix.json bgmVolume scales the whole bus.
    envelope = (
        "min(1,t/3)*"
        "if(lt(t,48),0.5,"
        " if(lt(t,52.5),0.5+0.5*(t-48)/4.5,"
        " if(lt(t,56.5),1.0,"
        " max(0,1.0-(t-56.5)/3.5))))"
    )
    run_ffmpeg([
        "-i", str(MUSIC_PATH),
...
def main() -> None:
    ...
    parser.add_argument("--voice", help="Override the CosyVoice voice for every CosyVoice line")
    ...
    # Check API keys per provider actually used by the spoken lines, before
    # any TTS call.
    spoken_providers = {
    ...
    for entry in entries:
        ...
        provider = "seedtts" if params.get("provider") == "seedtts" else "cosyvoice"
        if args.voice and provider == "cosyvoice":
        ...
        if args.force or not filepath.is_file():
            ...
            synthesize_line(dialogue, params, filepath, api_keys)
