...
export class Cheng extends MonkeyCharacter {
  ...
  buildHead(headGroup, furMat, skinMat) {
    ...
    // 微笑弧线嘴巴，放在脸下半部，明显像嘴而不是鼻子
    const curve = new THREE.CatmullRomCurve3([
      new THREE.Vector3(-0.08,  -0.165, 0.232),
      new THREE.Vector3(-0.04,  -0.152, 0.238),
      new THREE.Vector3(0,      -0.148, 0.240),
      new THREE.Vector3(0.04,   -0.152, 0.238),
      new THREE.Vector3(0.08,   -0.165, 0.232),
    ]);
    const mouth = new THREE.Mesh(
      new THREE.TubeGeometry(curve, 24, 0.014, 8, false),
      new THREE.MeshStandardMaterial({ color: 0x1a0a0a, roughness: 0.5 })
    );
    headGroup.add(mouth);
    this.mouth = mouth;
