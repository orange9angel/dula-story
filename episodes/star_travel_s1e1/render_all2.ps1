...
foreach ($start in $segments) {
    $dur = 15
    if (($start + $dur) -gt 195) { $dur = 195 - $start }
    if ($start -ge 195) { break }
    ...
    Write-Host "[RENDER] Segment ${start}s-$($start+$dur)s..."
...
$files | ForEach-Object { "file '$($_.FullName -replace '\\', '/')'" } | Out-File -FilePath $concatFile -Encoding utf8

Write-Host "[CONCAT] Merging $($files.Count) segments..."
...
Write-Host "[DONE] Final video: $episode\output\final.mp4"
