...
const boardDir = path.join(root, 'storyboard');
const audioFile = 'assets/audio/mixed.wav';
const outputFile = 'output/yuki_fish_musical.mp4';
const fps = 60;
...
if (!serveOnly) {
  ...
  try {
    ...
    for(let i=0;i<Math.round(duration*fps);i++) {
      ...
      performanceTrace.push({frame:i,...(capture?result.state:result)});
      if(capture) {
    ...
    fs.writeFileSync(path.join(boardDir,'performance_trace.json'),JSON.stringify({duration,fps,frames:performanceTrace}));
