...
DIALOGUE_PREPROCESSOR = {
    ...
    "等等。有动静。": "等等，有动静！",
    # Remove comma to prevent edge-tts from splitting into two segments
    "第一节，伸展运动。": "第一节伸展运动。",
}
...
def parse_story(text):
    ...
    while i < len(lines):
        ...
        # Strip {Character}{Animation|params} targeted animation tags
        dialogue = re.sub(r"\{\w+\}\{\w+(?:\|[^}]+)?\}\s*", "", dialogue)
        # Strip bare {Animation|params} tags (no colon, has pipe)
        dialogue = re.sub(r"\{\w+\|[^}]+\}\s*", "", dialogue)
...
def mix_bgm_track(cues, entries, duration, sample_rate=48000):
    ...
    # Save temp BGM track (no soft limiter, preserve dynamics)
...
def mix_audio(manifest, bgm_path=None, sfx_events=None):
    ...
    if entries:
        ...
        filter_file = os.path.join(OUTPUT_DIR, "_dialogue_filter_complex.txt")
        with open(filter_file, 'w') as f:
            f.write(filter_complex)
        cmd = f'ffmpeg -y {" ".join(inputs)} -filter_complex_script "{filter_file}" -map "[dialogue]" -acodec pcm_s16le -ar 48000 "{dialogue_path}"'
    ...
    if sfx_events:
        ...
        filter_file = os.path.join(OUTPUT_DIR, "_sfx_filter_complex.txt")
        with open(filter_file, 'w') as f:
            f.write(filter_complex)
        cmd = f'ffmpeg -y {" ".join(inputs)} -filter_complex_script "{filter_file}" -map "[sfxout]" -acodec pcm_s16le -ar 48000 "{sfx_path}"'
    ...
    # Mix without hard limiter - let the volumes breathe
    amix = f"{amix_inputs}amix=inputs={stream_idx}:duration=longest:normalize=0[outa];[outa]volume=0.95[limited]"
...
async def generate(force_tts=False):
    ...
    # Skip procedural SFX for broadcast exercise (no tennis/fighting needed)
    # Original: tennis_hit_times would generate "叮叮" sound effects
