#!/usr/bin/env python3
"""Generate CosyVoice dialogue, stems, and the full 128.5 s episode mix.
...
1. Parse script.story; synthesize every spoken entry ([Girl]/[Boy]).
...
4. Schedule SFX from script.story {SFX:Play} tags, build the BGM bus from
   assets/audio/music/warm_acoustic_bgm.mp3, and mix
   assets/audio/mixed.wav (130.4 s, 44.1 kHz) using config/audio_mix.json.
...
MORNING_MUSIC_PATH = OUTPUT_DIR / "music" / "umbrella_sun_theme.mp3"
RAIN_MUSIC_PATH = OUTPUT_DIR / "music" / "rain_gold_theme.mp3"
...
SFX_TRIM = {
    "inkstone_grind": (0.9, 0.2),
    "wind_gust": (1.5, 0.35),
    "umbrella_snap": (1.15, 0.25),
    "running_stone": (6.2, 0.45),
    "umbrella_open": (1.55, 0.3),
}
...
def build_bgm_bus() -> None:
    # A clean musical handoff at the rain beat: brighter ensemble guzheng for
    # the market, then the gentler "Ripple" cue for the umbrella and sunrise.
    run_ffmpeg([
        "-i", str(MORNING_MUSIC_PATH),
        "-stream_loop", "-1", "-i", str(RAIN_MUSIC_PATH),
        "-filter_complex",
        "[0:a]atrim=0:67,afade=t=in:st=0:d=2.2,afade=t=out:st=65.5:d=1.5,"
        "volume='if(lt(t,46.2),0.70,if(lt(t,55.5),0.82,0.72))':eval=frame[morning];"
        "[1:a]atrim=0:63,afade=t=in:st=0:d=1.5,afade=t=out:st=58.5:d=4.5,"
        "volume='if(lt(t,10),0.66,if(lt(t,35),0.74,if(lt(t,43.5),0.92,0.78)))':eval=frame,"
        "adelay=65500|65500[rain];"
        "[morning][rain]amix=inputs=2:duration=longest:normalize=0,"
        f"atrim=0:{DURATION_SECONDS}[bgmout]",
        "-map", "[bgmout]",
        "-acodec", "pcm_s16le", "-ar", str(SAMPLE_RATE), "-ac", "1",
        str(BGM_BUS_PATH),
    ])
...
def build_final_mix(mix_cfg: dict) -> None:
    ...
    if use_ducking:
        filters.append(
            f"[1:a]volume={bgm_vol}[bgmpre]"
        )
        filters.append(
            f"[bgmpre][0:a]sidechaincompress=threshold=0.018:ratio=5:attack=45:release=360:"
...
def main() -> None:
    ...
    for entry in entries:
        ...
        target = max(0.35, slot - 0.08)
        if effective > target:
            # Preserve pitch while tightening only the takes that would collide
            # with the next beat. This is intentionally restrained; the script
            # slots were written for conversational delivery, not speed-reading.
            tempo = min(1.45, effective / target)
            fitted = filepath.with_suffix(".fit.wav")
            print(f"  fitting {filename}: atempo={tempo:.3f} for {slot:.2f}s slot")
            run_ffmpeg([
                "-i", str(filepath),
