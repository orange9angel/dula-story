...
// Procedural golden dust-mote overlay, drawn over the keyframes and under the
// subtitles. Every mote is a pure function of (mote index, absolute time), so
// re-rendering the same frame always yields the identical image. Sunny
// counterpart to the rain overlay of rainy_rooftop_cat: slow warm particles
// drifting in the light, plus a few larger soft bokeh specks.
const MOTES = {
  count: 46,
  sizePx: [1.4, 3.6],
  speedPx: [12, 34], // upward drift
  swayPx: [10, 30], // horizontal sway amplitude
  alpha: 0.4,
  bokehCount: 8,
  bokehSizePx: [14, 34],
  bokehAlpha: 0.05,
...
// Picture-driven weather arc. The generated keyframes carry the large lighting
// changes; these deterministic overlays provide the continuous motion the eye
// expects between them. Values are tied to script.story, the timing authority.
const WEATHER = {
  rainStart: 65.5,
  rainFull: 69.0,
  rainEase: 95.0,
...
function parseStory(storyText) {
  ...
  if (!(duration > 0)) {
    throw new Error('HuazhongGurenSequenceScene could not read a positive duration from script.story');
  }
...
function normalizeTimeline(rawTimeline, storyDuration) {
  ...
  const frames = rawTimeline.frames.map((frame, index) => {
    ...
    return {
      ...
      eyeRig: frame.eyeRig == null ? [] : (Array.isArray(frame.eyeRig) ? frame.eyeRig.slice() : [frame.eyeRig]),
 ...
 */
export class HuazhongGurenScene extends SceneBase {
  constructor() {
    super('HuazhongGurenScene');
    this.timeline = [];
  ...
  build() {
    ...
    this.sequenceTexture.name = 'HuazhongGurenSequenceTexture';
    ...
    this.readyPromise = Promise.all([
      ...
      for (const frame of this.timeline) {
        ...
        for (const rigId of frame.eyeRig) {
          if (!this.eyeRigs.has(rigId)) {
            throw new Error(`Timeline shot ${frame.shot} references unknown eye rig ${rigId}`);
          }
        }
      }
  ...
  _drawAtTime(absoluteTime) {
    ...
    this._drawMotes(sequenceTime);
    this._drawPetals(sequenceTime);
    this._drawRain(sequenceTime);
    this._drawSunRays(sequenceTime);
    this._drawSubtitle(sequenceTime);
    ctx.restore();
    this.sequenceTexture.needsUpdate = true;
  }

  _drawMotes(sequenceTime) {
    // Gold pigment motes before the storm, then stronger pollen glints when the
    // sky clears. Keep the heavy-rain stretch visually clean.
    const rain = this._rainIntensityAt(sequenceTime);
    if (rain > 0.45) return;
    const phaseAlpha = sequenceTime < WEATHER.rainStart
      ? 0.72
      : smoothstep01((sequenceTime - WEATHER.sunStart) / 5.0);
    if (phaseAlpha <= 0.01) return;
    const ctx = this.sequenceContext;
    ctx.save();
    const travel = CANVAS_HEIGHT + 80;
    for (let index = 0; index < MOTES.count; index += 1) {
      const size = lerp(MOTES.sizePx[0], MOTES.sizePx[1], hash01(index * 3 + 1));
      const speed = lerp(MOTES.speedPx[0], MOTES.speedPx[1], hash01(index * 3 + 2));
      const baseX = hash01(index * 3 + 3) * CANVAS_WIDTH;
      const y = CANVAS_HEIGHT + 40 - ((hash01(index * 7 + 5) * travel + sequenceTime * speed) % travel);
      const sway = lerp(MOTES.swayPx[0], MOTES.swayPx[1], hash01(index * 5 + 2));
      const swayRate = lerp(0.3, 0.8, hash01(index * 5 + 4));
      const x = baseX + Math.sin(sequenceTime * swayRate + index * 1.7) * sway;
      const twinkle = 0.55 + 0.45 * Math.sin(sequenceTime * lerp(0.8, 1.8, hash01(index + 9)) + index);
      const alpha = MOTES.alpha * twinkle * phaseAlpha;
      if (alpha <= 0.03) continue;
      ctx.fillStyle = `rgba(255, 226, 170, ${alpha})`;
      ctx.beginPath();
      ctx.arc(x, y, size, 0, Math.PI * 2);
      ctx.fill();
    }
    // A few large, very soft out-of-focus specks for depth.
    for (let index = 0; index < MOTES.bokehCount; index += 1) {
      const size = lerp(MOTES.bokehSizePx[0], MOTES.bokehSizePx[1], hash01(index * 13 + 1));
      const speed = lerp(6, 14, hash01(index * 13 + 3));
      const baseX = hash01(index * 13 + 5) * CANVAS_WIDTH;
      const y = CANVAS_HEIGHT + 40 - ((hash01(index * 13 + 7) * travel + sequenceTime * speed) % travel);
      const x = baseX + Math.sin(sequenceTime * 0.25 + index * 2.3) * 24;
      ctx.fillStyle = `rgba(255, 234, 190, ${MOTES.bokehAlpha * phaseAlpha})`;
      ctx.beginPath();
      ctx.arc(x, y, size, 0, Math.PI * 2);
      ctx.fill();
    }
  ...
  _rainIntensityAt(sequenceTime) {
    if (sequenceTime < WEATHER.rainStart || sequenceTime >= WEATHER.rainEnd) return 0;
    if (sequenceTime < WEATHER.rainFull) {
      return smoothstep01((sequenceTime - WEATHER.rainStart) / (WEATHER.rainFull - WEATHER.rainStart));
    }
    if (sequenceTime <= WEATHER.rainEase) return 1;
    return 1 - smoothstep01((sequenceTime - WEATHER.rainEase) / (WEATHER.rainEnd - WEATHER.rainEase));
  }
  ...
  _drawRain(sequenceTime) {
    ...
    const ctx = this.sequenceContext;
    const slant = 0.18 + Math.sin(sequenceTime * 0.63) * 0.035;
    const travel = CANVAS_HEIGHT + 160;
    ctx.save();
    for (const layer of [
      { count: WEATHER.farDrops, speed: [540, 820], length: [18, 34], width: 1.0, alpha: 0.22 },
      { count: WEATHER.nearDrops, speed: [880, 1320], length: [34, 70], width: 1.8, alpha: 0.42 },
    ]) {
      ctx.strokeStyle = `rgba(198, 220, 240, ${layer.alpha * intensity})`;
      ctx.lineWidth = layer.width;
      ctx.beginPath();
      const visible = Math.floor(layer.count * intensity);
      for (let index = 0; index < visible; index += 1) {
        const speed = lerp(layer.speed[0], layer.speed[1], hash01(index * 13 + 1));
        const length = lerp(layer.length[0], layer.length[1], hash01(index * 13 + 3));
        const baseX = hash01(index * 13 + 5) * (CANVAS_WIDTH + 280) - 140;
        const y = ((hash01(index * 17 + 7) * travel + sequenceTime * speed) % travel) - 80;
        const x = baseX + y * slant;
        ctx.moveTo(x, y);
    ...
    // Expanding wet-stone/canal ripples make the static painted reflections
    // feel continuously struck by rain.
    for (let index = 0; index < Math.floor(WEATHER.rippleCount * intensity); index += 1) {
      const x = hash01(index * 19 + 2) * CANVAS_WIDTH;
      const y = CANVAS_HEIGHT * (0.68 + hash01(index * 19 + 4) * 0.28);
      const phase = (hash01(index * 19 + 6) + sequenceTime * lerp(0.55, 1.15, hash01(index + 8))) % 1;
      const radius = 3 + phase * 27;
      const alpha = (1 - phase) * 0.34 * intensity;
      ctx.strokeStyle = `rgba(210, 229, 246, ${alpha})`;
      ctx.lineWidth = 1.2;
      ctx.beginPath();
      ctx.ellipse(x, y, radius, radius * 0.28, 0, 0, Math.PI * 2);
      ctx.stroke();
    }
    ctx.restore();
  }

  _drawPetals(sequenceTime) {
    const beforeStorm = sequenceTime < 46.2;
    const afterStorm = sequenceTime >= WEATHER.rainEnd;
    if (!beforeStorm && !afterStorm) return;
    const alphaScale = beforeStorm ? 0.55 : 0.82;
    const ctx = this.sequenceContext;
    ctx.save();
    for (let index = 0; index < WEATHER.petalCount; index += 1) {
      const speedX = lerp(24, 58, hash01(index * 23 + 1));
      const speedY = lerp(8, 26, hash01(index * 23 + 3));
      const x = (hash01(index * 23 + 5) * (CANVAS_WIDTH + 160) + sequenceTime * speedX) % (CANVAS_WIDTH + 160) - 80;
      const y = (hash01(index * 23 + 7) * (CANVAS_HEIGHT + 100) + sequenceTime * speedY) % (CANVAS_HEIGHT + 100) - 50;
      const size = lerp(4, 9, hash01(index * 23 + 9));
      const rotation = sequenceTime * lerp(0.5, 1.4, hash01(index + 11)) + index;
      ctx.translate(x, y);
      ctx.rotate(rotation);
      ctx.fillStyle = `rgba(246, 224, 218, ${alphaScale * (0.45 + hash01(index + 17) * 0.4)})`;
      ctx.beginPath();
  ...
  _drawSunRays(sequenceTime) {
    if (sequenceTime < WEATHER.sunStart) return;
    const strength = smoothstep01((sequenceTime - WEATHER.sunStart) / (WEATHER.sunFull - WEATHER.sunStart));
    const ctx = this.sequenceContext;
    ctx.save();
    ctx.globalCompositeOperation = 'screen';
    const breathe = 0.88 + 0.12 * Math.sin(sequenceTime * 0.42);
    // The painted keyframes already contain directional sunbeams. Add only a
    // soft breathing flare here so the effect moves without creating visible
    // polygon edges across faces or costumes.
    ctx.globalAlpha = strength * breathe;
    const centerX = CANVAS_WIDTH * 0.47;
    const centerY = CANVAS_HEIGHT * 0.08;
    ...
    gradient.addColorStop(0, 'rgba(255, 246, 213, 0.14)');
    gradient.addColorStop(0.32, 'rgba(255, 226, 155, 0.065)');
  ...
  _renderShotSource(frame, sequenceTime) {
    ...
    for (const rigId of frame.eyeRig) {
      const rig = this.eyeRigs.get(rigId);
  ...
  _sampleMotion(frame, progress, sequenceTime) {
    ...
    // Breathing micro-zoom on any shot with a live face (mouth or eye rig),
    // not only dialogue shots — keeps close-ups feeling alive between lines.
    const dialogueBreath = (frame.mouthRig || frame.eyeRig.length > 0)
      ? Math.sin(sequenceTime * Math.PI * 0.72) * 0.0018
      : 0;
