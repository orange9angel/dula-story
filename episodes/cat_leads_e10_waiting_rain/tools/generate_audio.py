#!/usr/bin/env python3
"""E10《等雨停》audio chain: per-line seed-tts-2.0 dialogue (mp3), manifest,
dialogue/SFX/BGM buses, and the 76 s final mix.

Adapted from E08 volcano/tools/generate_audio_cosyvoice.py. E10 differences:

- All three characters (Girl/Boy/Cat) use ``provider: seedtts`` in
  config/voice_config.json, so only VOLC_SPEECH_API_KEY is required
  (source dula-story/.env.speech; the CosyVoice path is kept but unused).
- Per-line takes are kept as mp3 (manifest ``file`` points at the mp3);
  a 48 kHz mono PCM16 wav twin is kept alongside for measurement and buses.
- BGM is selected from the ``{Music:Play|name=...}`` tag in script.story
  (wonder_theme.wav, engine procedural) instead of "first file in music/",
  looped to the full timeline, with fadeIn/baseVolume/endTime honored.
- SFX-only story entries without an explicit ``endTime`` option fall back to
  the entry's own SRT end time (rain_shower 14-62 s, awning_drips 53-64 s).
- Final mix duration is 76.0 s (script.story timeline ends at 75.0 s).
