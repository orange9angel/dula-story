 ...
 *
 * move.sfx 支持三种写法：
 *   - 字符串：'energy_blast'（默认命中帧触发）
 *   - 对象：{ name: 'energy_blast', trigger: 'hitFrame', volume: 0.9 }
 *   - 数组：同一 move 可触发多个音效
...
export function registerLastDepositCombatActions() {
  ...
  registerCombatAction('dashShot', {
    moves: [
      ...
      { anim: 'PlasmaRifle', hitFrame: 0.20, sfx: { name: 'energy_blast', trigger: 'hitFrame', volume: 0.9 }, fx: { type: 'FXHitSpark', trigger: 'hitFrame', attach: 'defender' }, reaction: 'HitStagger', hitstop: 0.02, shake: 0.015, camera: 'FightImpact' },
      { anim: 'CrouchPlasmaRifle', hitFrame: 0.30, sfx: { name: 'energy_blast', trigger: 'hitFrame', volume: 0.9 }, fx: { type: 'FXHitSpark', trigger: 'hitFrame', attach: 'defender' }, reaction: 'HitStagger', hitstop: 0.02, shake: 0.015, camera: 'FightImpact' },
    ],
  });
  ...
  registerCombatAction('jumpShot', {
    moves: [
      ...
      { anim: 'CrouchPlasmaRifle', hitFrame: 0.30, sfx: { name: 'energy_blast', trigger: 'hitFrame', volume: 0.9 }, fx: { type: 'FXHitSpark', trigger: 'hitFrame', attach: 'defender' }, reaction: 'HitStagger', hitstop: 0.03, shake: 0.02, camera: 'FightImpact' },
    ],
  });
  ...
  registerCombatAction('droneShot', {
    moves: [
      { anim: 'PlasmaRifle', hitFrame: 0.18, sfx: { name: 'energy_blast', trigger: 'hitFrame', volume: 0.9 }, fx: { type: 'FXHitSpark', trigger: 'hitFrame', attach: 'defender' }, reaction: 'HitStagger', hitstop: 0.02, shake: 0.015, camera: 'FightImpact' },
    ],
  });
  ...
  registerCombatAction('dropBlow', {
    moves: [
      { anim: 'DashForward', hitFrame: null, sfx: { name: 'dash_whoosh', trigger: 'start', volume: 0.8 }, fx: { type: 'FXAfterImage', trigger: 'start', attach: 'attacker' }, camera: 'FightFollow' },
      { anim: 'PlasmaRifle', hitFrame: 0.20, sfx: { name: 'energy_blast', trigger: 'hitFrame', volume: 0.9 }, fx: [{ type: 'FXHitSpark', trigger: 'hitFrame', attach: 'defender' }, { type: 'FXShockwave', trigger: 'hitFrame', attach: 'defender' }], reaction: 'Knockdown', hitstop: 0.03, shake: 0.02, camera: 'FightImpact' },
    ],
  });
}
