...
import { sampleMouthCue } from 'dula-engine/lib/AudioMouthCue.js';
...
export class RobotCharacterBase extends CharacterBase {
  constructor(name) {
    ...
    // ──  facial system 兼容性设置 ──
    // CharacterBase/FacialAnimationSystem 使用 leftBrow/rightBrow，
    // 而 90 年代机甲角色 build() 里命名是 leftEyebrow/rightEyebrow，这里做别名统一。
    if (!this.leftBrow && this.leftEyebrow) {
      this.leftBrow = this.leftEyebrow;
      this.leftBrowBaseY = this.leftBrow.position.y;
    }
    if (!this.rightBrow && this.rightEyebrow) {
      this.rightBrow = this.rightEyebrow;
      this.rightBrowBaseY = this.rightBrow.position.y;
    }
    if (this.leftEyelid) this.leftEyelidBaseY = this.leftEyelid.position.y;
    if (this.rightEyelid) this.rightEyelidBaseY = this.rightEyelid.position.y;

    // mouth 在机甲角色里是一个 Group（包含 lowerLip），记录基准 scale/position。
    if (this.mouth) {
      this.mouthBaseScaleX = this.mouth.scale.x || 1;
      this.mouthBaseScaleY = this.mouth.scale.y || 1;
      this.mouthBaseScaleZ = this.mouth.scale.z || 1;
      this.mouthBaseY = this.mouth.position.y;
    }

    // 记录嘴唇基准位置，供嘴型动画使用。
    if (this.lowerLip) {
      this.lowerLip.userData.baseY = this.lowerLip.position.y;
      this.lowerLip.userData.baseScaleX = this.lowerLip.scale.x || 1;
      this.lowerLip.userData.baseScaleY = this.lowerLip.scale.y || 1;
      this.lowerLip.userData.baseScaleZ = this.lowerLip.scale.z || 1;
    }
    if (this.upperLip) {
      this.upperLip.userData.baseY = this.upperLip.position.y;
      this.upperLip.userData.baseScaleX = this.upperLip.scale.x || 1;
      this.upperLip.userData.baseScaleY = this.upperLip.scale.y || 1;
      this.upperLip.userData.baseScaleZ = this.upperLip.scale.z || 1;
    }
  }
  ...
  /**
   * 停止说话时还原嘴唇位置/缩放，避免嘴型停留在最后一帧。
   */
  stopSpeaking() {
    super.stopSpeaking();
    if (this.lowerLip) {
      this.lowerLip.position.y = this.lowerLip.userData.baseY ?? this.lowerLip.position.y;
      this.lowerLip.scale.set(
        this.lowerLip.userData.baseScaleX || 1,
        this.lowerLip.userData.baseScaleY || 1,
        this.lowerLip.userData.baseScaleZ || 1
      );
    }
    if (this.upperLip) {
      this.upperLip.position.y = this.upperLip.userData.baseY ?? this.upperLip.position.y;
      this.upperLip.scale.set(
        this.upperLip.userData.baseScaleX || 1,
        this.upperLip.userData.baseScaleY || 1,
        this.upperLip.userData.baseScaleZ || 1
      );
    }
  }

  /**
   * 机甲角色专用嘴型动画：覆盖 CharacterBase 的默认实现。
   * 默认实现把 visemeSequence 错当成 mouthCue 调用，导致嘴型永远只有开合、没有形变。
   * 这里根据文本 viseme + 音频 energy，驱动 lowerLip / upperLip 的位移与缩放。
   * 即使 visemeSequence 为空（例如 headless 环境 VisemeMapper 缺失），也会用音频/节奏 fallback 让嘴动起来。
   */
  _updateVisemeSequence(time) {
    if (!this.isSpeaking) return;

    let shape;
    if (this.visemeSequence && this.visemeSequence.length > 0) {
      shape = this.getCurrentMouthShape(time);
    } else {
      // Fallback：用 sinusoid + 音频 energy 生成明显的 syllable 脉冲
      const elapsed = time - this.speakStartTime;
      const cueSample = this.mouthCue ? sampleMouthCue(this.mouthCue, elapsed) : null;
      const energy = cueSample ? cueSample.energy : 0;
      const syllable = Math.abs(Math.sin(time * 14) * Math.cos(time * 5.2));
      const factor = Math.pow(syllable, 0.6) * (0.55 + energy * 0.45);
      shape = {
        lipHeight: 0.25 + 0.55 * factor,
        lipWidth: 1.0 + 0.35 * factor,
        jawOpen: 0.55 * factor,
        energy: 0.45 * factor,
        audioEnergy: energy,
      };
    ...
    const lipWidth = shape.lipWidth || 1;
    const energy = shape.audioEnergy || shape.energy || 0;

    if (this.lowerLip && this.upperLip) {
      // ── 下唇：随 jawOpen 下移 + 垂直拉长 + 宽度随 lipWidth 变化 ──
      const baseLowerY = this.lowerLip.userData.baseY ?? -0.012;
      const openDrop = jawOpen * 0.055 + energy * 0.022;
      this.lowerLip.position.y = baseLowerY - openDrop;
      this.lowerLip.scale.y = (this.lowerLip.userData.baseScaleY || 1) * (1 + jawOpen * 1.3);
      this.lowerLip.scale.x = (this.lowerLip.userData.baseScaleX || 1) * (1 + (lipWidth - 1) * 0.75);

      // ── 上唇：明显上提，配合开合更有表情 ──
      const baseUpperY = this.upperLip.userData.baseY ?? this.upperLip.position.y;
      this.upperLip.position.y = baseUpperY + jawOpen * 0.032;
      this.upperLip.scale.x = (this.upperLip.userData.baseScaleX || 1) * (1 + (lipWidth - 1) * 0.45);

      // ── 整嘴 Group 做明显开合呼吸，让嘴缝在特写中清晰可见 ──
      if (this.mouth) {
        this.mouth.scale.y = (this.mouthBaseScaleY || 1) * (1 + jawOpen * 0.55);
        this.mouth.scale.x = (this.mouthBaseScaleX || 1) * (1 + (lipWidth - 1) * 0.35);
      }
    } else if (this.mouth) {
      // 通用 mesh mouth：走 FacialAnimationSystem 的 viseme 表
      if (this.facialSystem) {
        const elapsed = time - this.speakStartTime;
        const seqInfo = (typeof window !== 'undefined' && window.VisemeMapper)
          ? window.VisemeMapper.getVisemeAtTime(this.visemeSequence, elapsed)
          : { viseme: 'CLOSED' };
        const visemeMap = { CLOSED: 'rest', A: 'aaa', I: 'eee', U: 'ooo', E: 'eee' };
        const visemeName = visemeMap[seqInfo.viseme] || 'rest';
        this.facialSystem.setViseme(visemeName, 0.4 + jawOpen * 0.6);
      }
    }
  }
