...
export class ReadableCombatDirector {
  ...
  _shotAim(ev) {
    ...
    play(attacker, 'ReadableAimRifle', ev.time, num(ev.options.fire, 0.75));
    play(attacker, 'PlasmaRifleCharge', ev.time, Math.min(0.7, num(ev.options.fire, 0.75)));
  ...
  _shotFire(ev) {
    ...
    play(attacker, 'ReadableFireRifle', ev.time, 0.45);
    play(attacker, 'PlasmaRifle', ev.time, 0.45);
    ...
    if (ev.options.block === true || ev.options.block === 'true' || ev.options.reaction === 'Block') {
      const shield = new TimedShield(end, c, 0.5);
      shield.addTo(this._scene());
      this.effects.push(shield);
      play(defender, 'Block', ev.time, 0.5);
    } else {
      play(defender, ev.options.reaction || 'HitStagger', ev.time, 0.65);
    }

    this._knockback(attacker, defender, ev.time, num(ev.options.knockback, 0.35), num(ev.options.knockTime, 0.28));
  ...
  _meleeHit(ev) {
    ...
    play(defender, ev.options.reaction || 'HitStagger', ev.time + 0.05, 0.65);
