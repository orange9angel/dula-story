...
export class PlasmaVaultScene extends SceneBase {
  ...
  build() {
    ...
    // 侧后方轮廓光：把深色反派角色从黑背景中分离
    const rimLight = new THREE.DirectionalLight(0x77bbff, 2.8);
    rimLight.position.set(0, 5, -12);
    this.scene.add(rimLight);

    // 右侧补光：避免深色材质（达什）完全沉入阴影
    const rightFill = new THREE.DirectionalLight(0x99ccff, 2.4);
    rightFill.position.set(6, 4, 8);
    this.scene.add(rightFill);

    // 维克专属补光：黑色/紫色材质吸光严重，额外点光勾勒轮廓
    const victorFill = new THREE.PointLight(0xcc99ff, 5.0, 14, 1.1);
    victorFill.position.set(2.2, 2.5, 3);
    this.scene.add(victorFill);

    // 摄影机方向补光：确保反派面向观众的正面不至于死黑
    const cameraFill = new THREE.DirectionalLight(0xccddee, 1.6);
    cameraFill.position.set(4, 3, 14);
    ...
    const glowTrimMat = new THREE.MeshBasicMaterial({ color: 0x00ccff, transparent: true, opacity: 0.20 });
    ...
    const ringMat = new THREE.MeshBasicMaterial({ color: 0x00ccff, side: THREE.DoubleSide, transparent: true, opacity: 0.30 });
    ...
    const coreMat = new THREE.MeshBasicMaterial({ color: 0x00ffff, transparent: true, opacity: 0.20, wireframe: true });
    ...
    // 核心点光源
    this.coreLight = new THREE.PointLight(0x00ffff, 2.2, 18, 1.2);
    ...
    for (let i = 0; i < 3; i++) {
      const ring = new THREE.Mesh(
        ...
        new THREE.MeshBasicMaterial({ color: 0x00ffff, transparent: true, opacity: 0.18 })
      );
    ...
    // 地面反射光
    const groundGlow = new THREE.PointLight(0x0088ff, 0.7, 12, 1.4);
  ...
  _createCeilingConduits() {
    ...
    const glowMat = new THREE.MeshBasicMaterial({ color: 0x00ccff, transparent: true, opacity: 0.12 });
  ...
  _createEnergyParticles() {
    ...
    const mat = new THREE.PointsMaterial({
      color: 0x00ffff, size: 0.04, transparent: true, opacity: 0.35,
  ...
  update(time, delta) {
    ...
    if (this.coreLight) {
      this.coreLight.intensity = 1.5 + Math.sin(time * 4) * 0.5;
    }
