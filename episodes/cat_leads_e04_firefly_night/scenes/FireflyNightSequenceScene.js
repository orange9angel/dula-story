...
// E04《夏夜流萤》 night post layers. Per STYLE_BIBLE.md every procedural layer
// is drawn as razor-clean flat vector shapes in source-image space (so the
// crop move carries them): zero grain, zero noise, no gradients, no blur.
// Colors come straight from the night palette: deep indigo sky, dark-purple
// night clouds, cool blue-silver moonlight, warm lamp accents.
const NIGHT = {
  skyIndigo: '#1B2A5E', // deep indigo night sky
  nightPurple: '#3A2E5C', // dark purple night haze / cloud body
  moonSilver: '#9FB8E8', // moonlight is a shape, never a glow
  moonBand: '#B8CCF0', // brighter cool silver for moonlight bands
  shadowNavy: '#3B3566', // deep lavender-navy shadow, never grey
  warmLight: '#FFD97A', // warm lamp / door light accents
  cloudBody: '#3A2E5C', // night clouds read as dark purple silhouettes
  cloudShadow: '#3B3566', // deeper lavender-navy shadow pass underneath
  steamWhite: '#B8CCF0', // cool silver wisps under moonlight
  fireflyGlow: '#E8F29A', // warm yellow-green firefly body
  fireflyCore: '#FFF7C0', // bright firefly core
};
...
function parseStory(storyText) {
  ...
  if (!(duration > 0)) {
    throw new Error('FireflyNightSequenceScene could not read a positive duration from script.story');
  }
...
function normalizeTimeline(rawTimeline, storyDuration) {
  ...
  const frames = rawTimeline.frames.map((frame, index) => {
    ...
    return {
      ...
      starTwinkle: normalizeStarTwinkle(frame.starTwinkle),
      fireflies: normalizeFireflies(frame.fireflies),
...
// "alpha": 0.32 } — flat cool-silver moonlight ellipses with a gentle
// sway/pulse (the night counterpart of E03's gold dappled sunlight).
...
// starTwinkle: { "stars": [[x, y, r, phaseHz], ...], "alpha": 0.6 } — flat
// moon-silver star dots in source-image space (same coordinate convention as
// dappleSway), each breathing in brightness at its own phaseHz: alpha stays
// within ±30% of the base alpha.
function normalizeStarTwinkle(raw) {
  if (!raw) return null;
  const stars = raw.stars;
  if (!Array.isArray(stars) || stars.length === 0) {
    throw new Error('starTwinkle config needs a non-empty stars array');
  }
  for (const star of stars) {
    if (
      ...
      throw new Error(`Invalid starTwinkle star: ${JSON.stringify(star)}`);
    }
  }
...
// fireflies: { "region": [x, y, w, h], "count": 24, "seed": 20260829,
// "size": 5, "alpha": 0.85, "drift": 30, "rise": 0, "period": [3.0, 6.0] } —
// warm yellow-green flat dots wandering the region rectangle on slow sine
// paths. Distribution and phases come from hash01(seed + index), so the same
// seed reproduces the same frame at the same sequenceTime; no setTimeout and
// no per-frame randomness. rise > 0 makes the swarm float upward at rise px/s
// and wrap inside the region.
function normalizeFireflies(raw) {
  if (!raw) return null;
  const region = raw.region?.map(Number);
  if (
    !region || region.length !== 4
    || region.some((v) => !Number.isFinite(v))
    || region[2] <= 0 || region[3] <= 0
  ) {
    throw new Error('fireflies config needs a four-value region with positive size');
  }
  const count = Number(raw.count ?? 24);
  if (!Number.isInteger(count) || count <= 0) {
    throw new Error('fireflies count must be a positive integer');
  }
  const seed = Number(raw.seed ?? 1);
  if (!Number.isFinite(seed)) throw new Error('fireflies seed must be numeric');
  const size = Number(raw.size ?? 5);
  if (!(size > 0)) throw new Error('fireflies size must be positive');
  const alpha = clamp(Number(raw.alpha ?? 0.85), 0.05, 1);
  const drift = Number(raw.drift ?? 30);
  if (!(drift >= 0)) throw new Error('fireflies drift must be >= 0');
  const rise = Number(raw.rise ?? 0);
  if (!(rise >= 0)) throw new Error('fireflies rise must be >= 0');
  const period = Array.isArray(raw.period) ? raw.period.map(Number) : [3.0, 6.0];
  if (period.length !== 2 || period.some((v) => !(v > 0))) {
    throw new Error('fireflies period must be two positive numbers');
  }
  return { region, count, seed, size, alpha, drift, rise, period };
}
...
function normalizeMouthRigs(rawRigs) {
  ...
  for (const [id, source] of Object.entries(rawRigs?.rigs || {})) {
    ...
    // Optional multi-cue binding (E04: cat rigs serve a meow SFX cue plus the
 ...
 * the E04《夏夜流萤》 night variant of the E03 keyframe-cel renderer.
 *
 ...
 *
 * All ambient motion (cloud drift, moonlight dapple sway, steam, the door
 * light band, star twinkle, fireflies) is procedural flat-shape drawing in
 * source-image space, so the 2D crop move carries every layer and re-renders
 * stay deterministic.
 */
export class FireflyNightScene extends SceneBase {
  constructor() {
    super('FireflyNightScene');
    this.timeline = [];
  ...
  build() {
    ...
    this.sequenceTexture.name = 'FireflyNightSequenceTexture';
  ...
  _renderShotSource(frame, sequenceTime) {
    ...
    // Night ambient layers, drawn in source-image space so the crop move
    // carries them. Order matters: sky (stars, clouds) first, foreground
    // light (door band, fireflies) last.
    if (frame.starTwinkle) this._drawStarTwinkle(frame.starTwinkle, sequenceTime);
    ...
    if (frame.fireflies) this._drawFireflies(frame.fireflies, sequenceTime);
  ...
  // Night bubble cloud: same flat-based body with four uneven bumps as E03,
  // but drawn as a dark purple silhouette with a deeper lavender-navy shadow
  // pass instead of white-on-lavender. Drifts right at cloud[4] px/s and
  // wraps across the source image; the phase is a pure function of absolute
  // time.
  _drawCloudDrift(config, sequenceTime, sourceWidth) {
    ...
    for (let index = 0; index < config.clouds.length; index += 1) {
      ...
      ctx.fillStyle = NIGHT.cloudShadow;
      ctx.globalAlpha = 0.32;
      cluster(ry * 0.34, 0.6);

      ctx.fillStyle = NIGHT.cloudBody;
  ...
  // Moonlight dapple: flat cool-silver ellipses (the night counterpart of the
  // E03 gold sunlight band) that sway a few pixels and pulse in alpha.
  // Per-spot period/phase come from hash01(index), so the pattern is stable
  // between renders.
  _drawDappleSway(config, sequenceTime) {
    const ctx = this.sourceContext;
    ctx.save();
    ctx.fillStyle = NIGHT.moonBand;
  ...
  _drawSteam(config, sequenceTime) {
    const ctx = this.sourceContext;
    ctx.save();
    ctx.fillStyle = NIGHT.steamWhite;
  ...
  // Door light band: a flat warm-lamp band that widens from the door center
  // as the door opens and collapses on the interior entry. At night the
  // interior light is the warm anchor against the cool moonlit street, timed
  // from the shot start so the chime and the band stay in sync.
  ...
  // Star twinkle: flat moon-silver dots, each breathing in brightness at its
  // own phaseHz within ±30% of the base alpha. The per-star phase offset is
  // hashed from the index, so the sky is stable between renders.
  _drawStarTwinkle(config, sequenceTime) {
    const ctx = this.sourceContext;
    ctx.save();
    ctx.fillStyle = NIGHT.moonSilver;
  ...
  // Fireflies: warm yellow-green flat dots with a brighter core, wandering
  // the region rectangle on slow sine paths. Positions, periods and blink
  // phases are all hash01(seed + index)-derived and driven purely by
  // sequenceTime, so one seed reproduces exactly one animation. rise > 0
  // lifts the whole swarm at rise px/s and wraps it inside the region.
  _drawFireflies(config, sequenceTime) {
    const ctx = this.sourceContext;
    const [regionX, regionY, regionW, regionH] = config.region;
    ctx.save();
    for (let index = 0; index < config.count; index += 1) {
      const base = config.seed + index * 101;
      const homeX = hash01(base + 1);
      const homeY = hash01(base + 2);
      const periodPick = hash01(base + 3);
      const phase = hash01(base + 4);
      const period = lerp(config.period[0], config.period[1], periodPick);
      const angle = (sequenceTime / period) * Math.PI * 2 + phase * Math.PI * 2;

      let x = regionX + homeX * regionW + Math.sin(angle) * config.drift;
      let y = regionY + homeY * regionH + Math.cos(angle * 0.77) * config.drift * 0.4;
      if (config.rise > 0) {
        const risen = (homeY * regionH - sequenceTime * config.rise) % regionH;
        y = regionY + ((risen % regionH) + regionH) % regionH;
      }

      // Slow per-firefly blink: brightness breathes on its own phase.
      const glow = 0.5 + 0.5 * Math.sin(angle * 1.3 + phase * Math.PI * 2);
      const alpha = config.alpha * (0.35 + 0.65 * glow);
      if (alpha <= 0.02) continue;
      const radius = config.size * (0.7 + 0.3 * glow);

      ctx.globalAlpha = alpha;
      ctx.fillStyle = NIGHT.fireflyGlow;
      ctx.beginPath();
      ctx.arc(x, y, radius, 0, Math.PI * 2);
      ctx.fill();

      ctx.globalAlpha = clamp(alpha * 1.2, 0, 1);
      ctx.fillStyle = NIGHT.fireflyCore;
      ctx.beginPath();
  ...
  _mouthStateAt(rig, sequenceTime) {
    // A rig may bind several cue entries (E04 cat: meow SFX cue + speech cue).
    // Later entries take precedence when both are audible at the same time.
