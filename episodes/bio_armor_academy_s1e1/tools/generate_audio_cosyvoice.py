#!/usr/bin/env python3
"""Generate CosyVoice dialogue, manifest, dialogue bus, and the full 59 s mix.
...
rainy_rooftop_cat episode mixer):

1. Parse script.story; synthesize every spoken entry
   ([LeiXiao]/[BaiLan]/[Narrator]).
...
4. Schedule SFX from script.story {SFX:Play} tags, build the BGM bus from
   assets/audio/music/inkblaze_theme.mp3, and mix
   assets/audio/mixed.wav (59.0 s, 44.1 kHz) using config/audio_mix.json.
...
# BGM 待定，先放占位:until the inkblaze theme track is picked, drop any
# temporary mix under assets/audio/music/inkblaze_theme.mp3.
MUSIC_PATH = OUTPUT_DIR / "music" / "inkblaze_theme.mp3"
...
# Per-SFX playback trims: (max_seconds, fade_out_seconds). Placeholder values
# pending the final SFX recordings; adjust once the sources land.
SFX_TRIM = {
    "wind_dusk": (10.0, 3.0),
    "crow_caws": (3.0, 0.8),
    "distant_traffic": (4.0, 1.0),
    "shinai_whoosh": (1.2, 0.3),
    "bio_impact_metal": (2.0, 0.5),
    "floor_crack": (2.0, 0.5),
    "heartbeat_core": (4.0, 1.5),
}
...
def build_bgm_bus() -> None:
    # Envelope: quiet bed under the schoolyard act, a lift into the 38 s
    # impact beat, and a fade-out across the final second. Gains here are
    # relative; config/audio_mix.json bgmVolume scales the bus at the mix.
    envelope = (
        "min(1,t/2)*"
        "if(lt(t,36),0.55,"
        " if(lt(t,38),0.55+0.45*(t-36)/2,"
        " if(lt(t,58),1.0,"
        " max(0,1.0-(t-58)))))"
    )
    run_ffmpeg([
