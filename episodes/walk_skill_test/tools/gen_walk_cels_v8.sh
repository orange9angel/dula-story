#!/bin/bash
# v8: frame_06 back-view walk A/B per the CORRECTED walk-director playbook
# (keyframe-walk-shots.md, post-v7): readability comes from DEPTH offset +
# trailing heel lift, NOT lateral spread. Feet stay near the body centerline
# (<= half shoe width apart) for a natural girlish gait; small stride.
# v7 (lateral spread >= one shoe width) rejected: straddle stance, no motion.
set -u
...
# Cel A: far foot steps a small step forward (toward the store) and bears
# weight; near foot trails behind with heel clearly lifted (push-off).
# frame_06 = composition + character lock ONLY.
gen frame_06_walk_contact_a_full-v8 "Use case: walk-cycle cel A (grounded
...
轻快。胯部可随承重腿轻微倾斜，其余上半身完全不动。她投在路面上的影子
的脚部轮廓跟随新的脚位变化（抬起的脚跟在影子里也抬起）。Output the
same pixel dimensions as image 1." \
  -i "$SRC/keyframes/frame_06.png"

# Cel B: exact opposite phase — the trailing foot steps forward to bear
# weight, the previously bearing foot trails with heel lifted.
gen frame_06_walk_contact_b_full-v8 "Use case: walk-cycle cel B, the exact
...
鞋尖朝道路前方，鞋轴沿道路透视；上半身完全保持原样。路面影子的脚部轮
廓跟随新的脚位（抬起的脚跟在影子里也抬起）。Output the same pixel
dimensions as image 1." \
  -i "$SRC/keyframes/frame_06.png" -i "$OUT/frame_06_walk_contact_a_full-v8.png"
