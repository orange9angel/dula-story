...
const check=args.includes('--check'),serve=args.includes('--serve'),mode=value('--mode','compare');
...
if(serve){console.log('Visual trial viewer ready');}else{
  ...
  try{
    ...
    const frames=Math.round(plan.duration*30),checkFrames=[0,18,36,57,79,96,110,128,149,168,186];
    const trace=[],hashes={},metrics={maxStanceDrift:0,maxLegLengthError:0,seekMatches:true,frames};
    if(!check){
      const file=path.join(output,mode==='compare'?'yuki_craft_comparison.mp4':'yuki_craft_after.mp4');
    ...
    for(let i=0;i<frames;i++){
      ...
      trace.push(state);
      for(const side of ['left','right'])if(prev&&prev.shot===state.shot&&prev[side+'Contact']&&state[side+'Contact'])metrics.maxStanceDrift=Math.max(metrics.maxStanceDrift,Math.hypot(state[side+'Foot'].x-prev[side+'Foot'].x,state[side+'Foot'].y-prev[side+'Foot'].y));
      prev=state;
      const s=shots.find(s=>t>=s.start&&t<s.end)??shots.at(-1),p=samplePose(t-s.start,s.kind,s.end-s.start);
      for(const side of ['left','right']){const [a,b,c]=p[side+'Leg'];metrics.maxLegLengthError=Math.max(metrics.maxLegLengthError,Math.abs(Math.hypot(b.x-a.x,b.y-a.y)-.305),Math.abs(Math.hypot(c.x-b.x,c.y-b.y)-.295));}
    ...
    for(const t of [1.2,4.27])fs.writeFileSync(path.join(board,`silhouette_${t.toFixed(2)}.png`),Buffer.from(await page.evaluate(t=>window.pngAt(t),t),'base64'));
