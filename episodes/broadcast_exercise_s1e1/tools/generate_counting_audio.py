...
def save_counting_outputs(track, output_dir):
    """Write counting to both music and sfx so either cue type can reference it."""
    save_wav(track, os.path.join(output_dir, 'counting.wav'))

    sfx_dir = os.path.join(os.path.dirname(output_dir), 'sfx')
    os.makedirs(sfx_dir, exist_ok=True)
    save_wav(track, os.path.join(sfx_dir, 'counting.wav'))
...
async def main():
    ...
    save_counting_outputs(track, output_dir)
