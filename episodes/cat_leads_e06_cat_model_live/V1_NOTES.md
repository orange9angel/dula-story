...
**第 2 步 · 生产锚点（~¥10–15）**：按定调稿生成角色建立视频
（~¥5/角色），监制验收脸与纹理（第二道门）。**定稿脸进生产的三条路径**
（导演 2026-09-06 提出"静帧定调≠T2V 还原"问题后补充）：
- **C（首选）**：定稿角色照（合成进定稿场景）→ DreamActor 驱动成建立
  视频——脸就是定稿那张；需先验证 DA 输出可作 reference_video（下周
  第一条命令）。选 C 而非换脸的理由（导演 2026-09-06 确认）：与全管线
  同为云 API 无本地模型依赖；DA 保整头（发际线/头型/背景一体），换脸
  只换面部区域且侧脸衰减。
- **B（兜底）**：T2V 生成 + 本地 facefusion 换脸（开源零边际成本，需部署）。
- **A（非关键角色）**：T2V 抽卡逼近定调稿（~¥5/roll × 3–5）。
纹理纪律：全部镜头走视频模型生成（环境镜若 2.0/2.5 纹理差肉眼可辨则
统一 2.5）；对齐用音频加垫，禁视频变速。
