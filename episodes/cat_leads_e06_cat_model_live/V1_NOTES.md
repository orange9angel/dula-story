...
2. **主体库 API 验证**（导演 2026-09-06 提议）：即梦 CV"主体识别/检测"
   ...
   avatar 家族产品定位吃人脸图片，可能出现最省路径：**文生图像素级定稿
   → 登记落户 → 生成引用 role_id**。验证：AI 生成脸是否被接受、主体可
   引用于哪些生成接口（仅 OmniHuman 数字人 = 说话镜专用户口；能喂视频
   生成 = 全片通用户口）。决策树：图片"方向不错"→ 内部定稿（视频链）；
   "非这张脸不可"→ DA；长期持有 → 主体库优先。
