...
function parseStory(storyText) {
  ...
  if (!(duration > 0)) {
    throw new Error('XiaojuSecretSequenceScene could not read a positive duration from script.story');
  }
 ...
 */
export class XiaojuSecretScene extends SceneBase {
  constructor() {
    super('XiaojuSecretScene');
    this.timeline = [];
  ...
  build() {
    ...
    this.sequenceTexture.name = 'XiaojuSecretSequenceTexture';
  ...
  _drawMotes(sequenceTime) {
    ...
    for (let index = 0; index < MOTES.count; index += 1) {
      ...
      const alpha = MOTES.alpha * twinkle;
    ...
    for (let index = 0; index < MOTES.bokehCount; index += 1) {
      ...
      ctx.fillStyle = `rgba(255, 234, 190, ${MOTES.bokehAlpha})`;
      ctx.beginPath();
      ctx.arc(x, y, size, 0, Math.PI * 2);
      ctx.fill();
    }
