#!/usr/bin/env python3
"""Generate CosyVoice dialogue, manifest, dialogue bus, and the full 130.4 s mix.
...
MUSIC_PATH = OUTPUT_DIR / "music" / "warm_acoustic_bgm.mp3"
...
def build_bgm_bus() -> None:
    # Envelope: gentle morning bed, a small lift when the riverside opens at
    # 57.8 s, the emotional swell at the sketchbook reveal at 105.4 s, and a
    # fade-out across the final 2.4 s. Gains here are relative;
    # config/audio_mix.json bgmVolume scales the whole bus at the final mix.
    envelope = (
        "min(1,t/2)*"
        "if(lt(t,55.8),0.5,"
        " if(lt(t,57.8),0.5+0.15*(t-55.8)/2,"
        " if(lt(t,103.4),0.65,"
        " if(lt(t,105.4),0.65+0.35*(t-103.4)/2,"
        " if(lt(t,128.0),1.0,"
        " max(0,1.0-(t-128.0)/2.4))))))"
    )
    run_ffmpeg([
        "-i", str(MUSIC_PATH),
