...
gen frame_18 "Use case: keyframe (base for a lip-sync talking shot). Close-up under the willow: $OLDMAN He faces the river again (frame LEFT), voice lowering; expression: eyebrows slightly drawn together; eyes narrowed toward the water, a little distant; mouth in a faint closed line. The straw hat brim shades his eyes slightly. Composition: 16:9 landscape, old man on the right half, calm river to the left." \
  -i "$A/oldman_reference.png" -i "$K/frame_13.png"

gen frame_19 "Use case: keyframe, reaction. Medium shot: $GIRL and $BOY turn their heads toward each other and exchange a puzzled glance, eyebrows raised, mouths closed. The old man is out of frame. Composition: 16:9 landscape, girl left, boy right, faces close to center." \
...
gen frame_21 "Use case: keyframe, FORESHADOW beat (deliberately held). Medium shot at the shoreline away from the group: $CAT The cat sits alone at the water's edge on the LEFT third, body facing frame RIGHT, perfectly still, staring fixedly toward the hazy distant hills upstream (the far background-right); its tail wrapped tight, ears up -- an unnaturally focused posture. Nobody else in frame. Composition: 16:9 landscape, cat small on the left, long empty river and far hills fill the frame." \
...
gen frame_23 "Use case: keyframe (base for a lip-sync talking shot). Close-up at the shoreline: $CAT The cat has turned its head back toward the camera-left (mouth clearly visible, closed), body still facing the river; expression: eyelids lowered, gaze calm and deliberately neutral, ears relaxed -- as if nothing happened. Composition: 16:9 landscape, cat on the right third, river glint behind." \
