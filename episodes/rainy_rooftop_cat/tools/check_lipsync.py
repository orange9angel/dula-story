...
def main() -> int:
    ...
    for index, cue in cue_entries.items():
        ...
        if alignment not in ("energy-gated-text-viseme-v1", "energy-gated-sfx-v1"):
            errors.append(f"Cue entry {index} has unknown alignment {alignment!r}")
        if alignment == "energy-gated-text-viseme-v1" and not cue.get("syllables"):
    ...
    rain_pose_count = sum(1 for time in times if 0.0 <= time < 19.5)
    clearing_pose_count = sum(1 for time in times if 19.5 <= time < 31.0)
    print(
        f"Checked {len(frames)} timeline beats, {len(rigs)} mouth rigs, "
        f"{len(cue_entries)} audio cues, {len(image_size_cache)} images."
    )
    print(f"Act cadence: rain poses={rain_pose_count}, clearing poses={clearing_pose_count}")
    if errors:
        for error in errors:
            print(f"ERROR: {error}")
        return 1
