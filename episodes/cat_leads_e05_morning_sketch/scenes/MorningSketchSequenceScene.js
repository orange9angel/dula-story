...
// E05《清晨速写》 morning post layers. Per STYLE_BIBLE.md every procedural
// layer is drawn as razor-clean flat vector shapes in source-image space (so
// the crop move carries them): zero grain, zero noise, no gradients, no blur.
// Colors come straight from the morning palette: cyan-blue sky top, peach-gold
// horizon, a brighter flat gold band low over the horizon, a low pale-gold
// sun disc, lavender shadow purple, flat white dew sparkles.
const MORNING = {
  skyTop: '#3E9BD8', // fresh morning cyan blue at the top of the sky
  horizonGold: '#FFC07A', // peach gold softening toward the horizon
  lowGold: '#F5B942', // brighter flat gold band low over the horizon
  sunDisc: '#FFD97A', // low flat pale-gold sun disc, hard-edged, no halo
  shadowPurple: '#8E7CC3', // lavender purple shadow side, never grey
  dewWhite: '#FDFBF4', // flat white four-point dew sparkles
  cloudBody: '#FDFBF4', // morning clouds read as flat white silhouettes
  cloudShadow: '#8E7CC3', // lavender-purple shadow pass underneath
  dappleGold: '#FFD97A', // flat pale-gold morning light dapple
  steamWhite: '#FDFBF4', // warm white wisps in morning light
  doorLight: '#F5B942', // flat gold light band spilling from the door
  warmLight: '#FFD97A', // warm pale-gold edge accents
  birdTeal: '#2E6B8A', // flat deep teal flyby birds
};
...
function parseStory(storyText) {
  ...
  if (!(duration > 0)) {
    throw new Error('MorningSketchSequenceScene could not read a positive duration from script.story');
  }
...
function normalizeTimeline(rawTimeline, storyDuration) {
  ...
  // Ambience beds and the closing fade run past the last subtitle cue, so the
  // timeline may declare a duration longer than the story's last cue end; the
  // declared duration is the authoritative sequence length.
  ...
  const frames = rawTimeline.frames.map((frame, index) => {
    ...
    return {
      ...
      dewSparkle: normalizeDewSparkle(frame.dewSparkle),
      birdsFlyby: normalizeBirdsFlyby(frame.birdsFlyby),
...
// "alpha": 0.32 } — flat pale-gold morning light ellipses with a gentle
// sway/pulse (same language as E03's gold dappled sunlight).
...
// flat soft-edged wisps rising from warm breakfast pots and cups. Same
// emitter shape as the reference episode's drips config.
...
// band that widens from the door center as the front door opens, and
// collapses again on the entry shot.
...
// dewSparkle: { "stars": [[x, y, r, phaseHz], ...], "alpha": 0.6 } — flat
// white four-point dew sparkles on grass and leaf tips in source-image space
// (same coordinate convention as dappleSway), each breathing in brightness at
// its own phaseHz: alpha stays within ±30% of the base alpha.
function normalizeDewSparkle(raw) {
  if (!raw) return null;
  const stars = raw.stars;
  if (!Array.isArray(stars) || stars.length === 0) {
    throw new Error('dewSparkle config needs a non-empty stars array');
  }
  for (const star of stars) {
    if (
      !Array.isArray(star) || star.length !== 4
      || star.some((v) => !Number.isFinite(Number(v)))
      || Number(star[2]) <= 0 || Number(star[3]) <= 0
    ) {
      throw new Error(`Invalid dewSparkle star: ${JSON.stringify(star)}`);
    }
  }
  const alpha = clamp(Number(raw.alpha) || 0.6, 0.05, 1);
  return { stars: stars.map((s) => s.map(Number)), alpha };
}

// birdsFlyby: { "count": 5, "y": 0.18, "startAt": 53.5, "duration": 4,
// "size": 14, "spacing": 46, "flapHz": 3.2 } — a small flock of flat
// deep-teal V-shaped birds crossing the sky from the left edge to the right
// edge at constant speed. y is a fraction of the source image height;
// startAt is absolute sequence seconds. The flight is a pure function of
// sequenceTime: before startAt and after startAt + duration nothing is
// drawn (once off screen the flock never reappears).
function normalizeBirdsFlyby(raw) {
  if (!raw) return null;
  const count = Number(raw.count ?? 5);
  if (!Number.isInteger(count) || count <= 0) {
    throw new Error('birdsFlyby count must be a positive integer');
  }
  const y = Number(raw.y);
  if (!Number.isFinite(y) || y < 0 || y > 1) {
    throw new Error('birdsFlyby y must be a 0..1 fraction of the source height');
  }
  const startAt = Number(raw.startAt);
  if (!Number.isFinite(startAt) || startAt < 0) {
    throw new Error('birdsFlyby startAt must be >= 0');
  }
  const duration = Number(raw.duration);
  if (!(duration > 0)) throw new Error('birdsFlyby duration must be positive');
  const size = Number(raw.size ?? 14);
  if (!(size > 0)) throw new Error('birdsFlyby size must be positive');
  const spacing = Number(raw.spacing ?? 46);
  if (!(spacing > 0)) throw new Error('birdsFlyby spacing must be positive');
  const flapHz = Number(raw.flapHz ?? 3.2);
  if (!(flapHz > 0)) throw new Error('birdsFlyby flapHz must be positive');
  return { count, y, startAt, duration, size, spacing, flapHz };
}
...
function normalizeMouthRigs(rawRigs) {
  ...
  for (const [id, source] of Object.entries(rawRigs?.rigs || {})) {
    ...
    // Optional multi-cue binding (a rig may serve an SFX cue plus the
    // following speech cue); later entries win when cues overlap in time.
    const extraEntries = Array.isArray(source.entries)
      ? source.entries.map(Number).filter(Number.isInteger)
      : null;
    if (source.mode === 'procedural') {
      ...
      rigs.set(id, { id, entry, entries: extraEntries, mode: 'procedural', anchor, size });
      continue;
    }
    const rect = source.rect?.map(Number);
    if (!rect || rect.length !== 4 || rect.some((value) => !Number.isFinite(value))) {
    ...
    rigs.set(id, { id, entry, entries: extraEntries, mode: 'image', rect, variants });
  }
  return rigs;
}
 ...
 * the E05《清晨速写》 morning variant of the E03 keyframe-cel renderer.
 *
 ...
 *
 * All ambient motion (cloud drift, morning light dapple sway, steam, the door
 * light band, dew sparkle, bird flyby) is procedural flat-shape drawing in
 * source-image space, so the 2D crop move carries every layer and re-renders
 * stay deterministic.
 */
export class MorningSketchScene extends SceneBase {
  constructor() {
    super('MorningSketchScene');
    this.timeline = [];
  ...
  build() {
    ...
    this.sequenceTexture.name = 'MorningSketchSequenceTexture';
  ...
  _renderShotSource(frame, sequenceTime) {
    ...
    if (frame.mouthRig) {
      ...
      const state = this._mouthStateAt(rig, sequenceTime);
    ...
    // Morning ambient layers, drawn in source-image space so the crop move
    // carries them. Order matters: sky (clouds, birds) first, foreground
    // sparkle (dew, door band) last.
    if (frame.cloudDrift) this._drawCloudDrift(frame.cloudDrift, sequenceTime, width);
    if (frame.birdsFlyby) this._drawBirdsFlyby(frame.birdsFlyby, sequenceTime, width, height);
    ...
    if (frame.dewSparkle) this._drawDewSparkle(frame.dewSparkle, sequenceTime);
  ...
  // Morning bubble cloud: same flat-based body with four uneven bumps as E03,
  // drawn as a flat white silhouette with a lavender-purple shadow pass
  // underneath. Drifts right at cloud[4] px/s and wraps across the source
  // image; the phase is a pure function of absolute time.
  _drawCloudDrift(config, sequenceTime, sourceWidth) {
    ...
    for (let index = 0; index < config.clouds.length; index += 1) {
      ...
      ctx.fillStyle = MORNING.cloudShadow;
      ctx.globalAlpha = 0.32;
      cluster(ry * 0.34, 0.6);

      ctx.fillStyle = MORNING.cloudBody;
  ...
  // Morning light dapple: flat pale-gold ellipses (same language as the E03
  // gold sunlight band) that sway a few pixels and pulse in alpha. Per-spot
  // period/phase come from hash01(index), so the pattern is stable between
  // renders.
  _drawDappleSway(config, sequenceTime) {
    const ctx = this.sourceContext;
    ctx.save();
    ctx.fillStyle = MORNING.dappleGold;
  ...
  // softness, staying inside the flat-shape language.
  _drawSteam(config, sequenceTime) {
    const ctx = this.sourceContext;
    ctx.save();
    ctx.fillStyle = MORNING.steamWhite;
  ...
  // Door light band: a flat gold band that widens from the door center as the
  // front door opens and collapses on the entry shot. In the morning the
  // interior light is a warm anchor against the cool blue outdoor shadows,
  // timed from the shot start so the door SFX and the band stay in sync.
  ...
  // Dew sparkle: flat white four-point star shapes on grass and leaf tips,
  // each breathing in brightness at its own phaseHz within ±30% of the base
  // alpha. The per-sparkle phase offset is hashed from the index, so the
  // meadow is stable between renders.
  _drawDewSparkle(config, sequenceTime) {
    const ctx = this.sourceContext;
    ctx.save();
    ctx.fillStyle = MORNING.dewWhite;
    for (let index = 0; index < config.stars.length; index += 1) {
      const [x, y, r, phaseHz] = config.stars[index];
      const phaseOffset = hash01(index * 31 + 11) * Math.PI * 2;
      const breathe = Math.sin(sequenceTime * phaseHz * Math.PI * 2 + phaseOffset);
      const alpha = config.alpha * (1 + 0.3 * breathe);
      if (alpha <= 0.02) continue;
      ctx.globalAlpha = clamp(alpha, 0, 1);
      // Hard-edged four-point sparkle: a tall/thin diamond cross, flat fill.
      const w = r * 0.28;
      ctx.beginPath();
      ctx.moveTo(x, y - r);
      ctx.lineTo(x + w, y - w);
  ...
  // Bird flyby: a small flock of flat deep-teal V-shaped birds crossing the
  // sky left-to-right at constant speed during the configured time window.
  // Positions and wing flap are pure functions of sequenceTime, so re-renders
  // stay deterministic; outside [startAt, startAt + duration] the flock is
  // fully off screen and nothing is drawn.
  _drawBirdsFlyby(config, sequenceTime, sourceWidth, sourceHeight) {
    const progress = (sequenceTime - config.startAt) / config.duration;
    if (progress < 0 || progress > 1) return;
    const ctx = this.sourceContext;
    const margin = config.size * 4 + config.spacing * config.count;
    const leadX = -margin + (sourceWidth + margin * 2) * progress;
    const baseY = config.y * sourceHeight;
    ctx.save();
    ctx.strokeStyle = MORNING.birdTeal;
    ctx.lineCap = 'round';
    ctx.lineWidth = Math.max(1.6, config.size * 0.18);
    for (let index = 0; index < config.count; index += 1) {
      // V formation: each trailing bird drops back and alternates sides.
      const rank = Math.ceil(index / 2);
      const side = index % 2 === 0 ? 1 : -1;
      const x = leadX - rank * config.spacing;
      const y = baseY + side * rank * config.spacing * 0.45;
      const flap = Math.sin(sequenceTime * config.flapHz * Math.PI * 2 + index * 1.3);
      const wing = config.size * (0.55 + 0.45 * flap);
      ctx.beginPath();
      ctx.moveTo(x - config.size, y - wing);
      ctx.lineTo(x, y);
      ctx.lineTo(x + config.size, y - wing);
      ctx.stroke();
    }
    ctx.restore();
  }

  _mouthStateAt(rig, sequenceTime) {
    // A rig may bind several cue entries (e.g. a meow SFX cue plus the
    // following speech cue). Later entries take precedence when both are
    // audible at the same time.
    const entryIndices = Array.isArray(rig.entries) && rig.entries.length
      ? [...rig.entries].reverse()
      : [rig.entry];
    for (const entryIndex of entryIndices) {
      ...
      if (state === 'open' || state === 'half') return state;
    }
    return 'closed';
  }
